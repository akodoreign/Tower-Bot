...
class DiscordClient(discord.Client):
    ...
    async def mission_board_loop(self):
        ...
        while not self.is_closed():
            ...
            try:
                from src.db_api import get_global_state, set_global_state
                _claim_queue = get_global_state("dashboard_claim_queue") or []
                if _claim_queue and isinstance(_claim_queue, list):
                    _item = _claim_queue.pop(0)
                    set_global_state("dashboard_claim_queue", _claim_queue)
                    try:
                        from src.mission_board import handle_dashboard_claim
                        await handle_dashboard_claim(
                            int(_item["mission_id"]),
                            _item.get("claimer", "DM Dashboard"),
                            client=self,
                        )
                    except Exception as _claim_e:
                        logger.warning(f"⚔️ Dashboard claim queue item failed: {_claim_e}")

                _queue = get_global_state("module_gen_queue") or []
                if _queue and isinstance(_queue, list):
                    _active = get_global_state("module_gen_active")
                    if not _active:
                        _item = _queue.pop(0)
                        set_global_state("module_gen_queue", _queue)
                        set_global_state("module_gen_active", _item["mission_id"])
                        from src.mission_board import _load_missions
                        _missions = _load_missions()
                        _mission = next((m for m in _missions if m.get("id") == _item["mission_id"]), None)
                        if _mission:
                            _claimer = _item.get("claimer", "DM Dashboard")
                            _client_ref = self
                            from src.cogs.module_gen import generate_and_post_module
                            async def _run_gen(_m=_mission, _c=_claimer):
                                try:
                                    await generate_and_post_module(_m, _c, _client_ref)
                                finally:
                                    await asyncio.to_thread(set_global_state, "module_gen_active", None)
                            asyncio.get_running_loop().create_task(_run_gen())
                            logger.info(f"📖 Module gen started from dashboard: '{_mission.get('title','?')}' for {_claimer}")
                        else:
                            logger.warning(f"📖 Module gen queue: mission ID {_item['mission_id']} not found in DB")
                            set_global_state("module_gen_active", None)
    ...
    async def npc_portrait_loop(self):
        """Post NPC character bio portraits every 5-7 hours — separate from scene images."""
        import discord as _discord
        import io, random
        ...
        while not self.is_closed():
            ...
            # Every 2.5-3.5 hours — offset from the 2-4h scene loop
            interval = random.randint(int(2.5 * 3600), int(3.5 * 3600))
