    ...
    if event_type == "construction":
        ...
        if is_susp:
            ...
            if risk <= 50:
                ...
                bulletin_text = (
                    f"**â›ª UNEXPLAINED OUTREACH — {district_a.upper()}**\n"
                ...
                bulletin_text = (
                    f"**â›ª RELIGIOUS OUTREACH — {district_a.upper()}**\n"
            ...
            logger.info(
                f"â›ª Suspicious religious outreach: {god_name} in {district_a} "
...
def _build_world_lore() -> str:
    ...
    Sources:
      global_state â†’ world_setting, world_currency, world_gods, world_key_npcs,
                     world_active_tensions
      faction_reputation â†’ live faction list with descriptions, leaders, rep tier
...
def _dual_timestamp() -> str:
    ...
    return f"{real_str} â”‚ Tower: {tower_str}"
    ...
    try:
        ...
        data = await call_ollama(
            payload={
                ...
                "stream": False,
                "options": {"num_predict": 1024, "num_ctx": 12288, "think": True},
            },
            timeout=600.0,
            caller="news_types",
        )
...
def _load_live_roster() -> List[Dict]:
    ...
    try:
        from src.db_api import raw_query as _rq
        rows = _rq("SELECT name, faction, role, status, location, data_json FROM npcs WHERE status IN ('alive','injured','undead','doppelganger') ORDER BY name") or []
        if rows:
            result = []
            for row in rows:
...
âœ— "X EC per person/life/soul/head" — FORBIDDEN (e.g. "120 EC per person smuggled" is WRONG)
âœ— "X EC per [victim count/abstract]" — FORBIDDEN
âœ— "X EC in [abstract]" — FORBIDDEN (e.g. "47 EC in shattered glass" is WRONG)
âœ— "X EC worth of [emotion/chaos/despair]" — FORBIDDEN
âœ— "bleeding EC" / "EC from fingertips" — FORBIDDEN
âœ— "the weight of X EC" (when not literal) — FORBIDDEN
âœ— EC as counting victims, smuggled people, or lives — ABSOLUTELY FORBIDDEN
âœ— "X EC reward/bounty for [info/evidence]" — FORBIDDEN. Rewards are Kharma-based or specific goods, NOT abstract EC amounts.
âœ— "X EC increments" — FORBIDDEN. Don't count things in EC units.
âœ— "smuggled EC" / "trading EC" — FORBIDDEN. EC is not a commodity to be smuggled or traded between NPCs.
âœ— "recovery costs in X EC" — FORBIDDEN. Don't use EC for abstract cost counting.
âœ— "X EC for the shard/ledger/reward" — FORBIDDEN when the shard/ledger is not an actual good being purchased.

ALLOWED EC PATTERNS (ONLY these are acceptable):
âœ“ "a sword costs 150 EC" — selling a specific tangible good
âœ“ "she paid 75 EC for information" — paying a specific informant for a specific thing
âœ“ "the smithy wants 200 EC for repairs" — specific service with a named provider
âœ“ "stole 300 EC from the warehouse" — actual theft of currency itself
âœ“ "vials cost 30 EC each" — specific goods with a price
    ...
    try:
        ...
        data = await call_ollama(
            payload={
                ...
                "stream": False,
                "options": {"num_predict": 1024, "num_ctx": 8192, "think": True},
            },
            timeout=600.0,
        ...
        if words_ok and lines_ok:
            logger.info("âœ… Fact-check: bulletin validated/corrected")
            return result
        else:
            logger.warning(
    ...
    try:
        ...
        data = await call_ollama(
            payload={
                ...
                "stream": False,
                "options": {"num_predict": 1200, "num_ctx": 8192, "think": True},
            },
            timeout=300.0,
            caller="editor",
        )
    ...
    try:
        ...
        data = await call_ollama(
            payload={
                ...
                "messages": [{"role": "user", "content": prompt}],
                "stream": False,
                "options": {"num_predict": 900, "num_ctx": 8192, "think": False},
            },
            timeout=600.0,
...
def _load_sd_prompts() -> dict:
    """Load NPC name â†’ SD prompt lookup from MySQL npc_appearances (falls back to file)."""
    try:
        from src.db_api import raw_query as _rq
...
async def _prompt_agent(
    ...
    if is_anime_local:
        ...
        agent_user = (
            ...
            f"(story event FIRST â†’ location â†’ atmosphere â†’ characters last)."
        )
    ...
    try:
        ...
        data = await call_ollama(
            payload={
                ...
                "stream": False,
                "options": {"num_predict": 256, "num_ctx": 8192, "think": True},
            },
            timeout=45.0,
            caller="image_tags",
        )
    ...
    # Photorealistic: agent already ordered environment â†’ story â†’ character (brief).
...
async def generate_npc_portrait() -> tuple:
    ...
    _is_player_char = random.random() < 0.30  # 30% chance it's a player character portrait

    sd_appearance = ""
    npc_name = npc_faction = npc_rank = npc_role = npc_loc = ""
    ...
    _is_alt_universe = random.random() < 0.20
