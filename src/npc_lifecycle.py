...
async def apply_npc_event(npc: dict, all_npcs: List[dict]) -> tuple:
    ...
    # --- INJURY -> DEATH ---
        ...
        if _return_roll < 0.40:
            ...
            npc["status"] = "alive"
            npc["history"].append(f"[{today}] Returned from death via Tower Fund Me. Alive again, for a price.")
            ...
            npc["history"].append(f"[{today}] A doppelganger wearing {name}'s face has appeared. The original is still dead.")
            new_secret = await _generate(
            ...
            npc["status"] = "undead"
            npc["history"].append(f"[{today}] Returned as undead. Reanimated. Not the same.")
            new_secret = await _generate(
...
- No preamble, no sign-off. Output the mission post only."""
            ...
            # === DOPPELGANGER — Someone is impersonating the dead NPC ===
                ...
                dead_npc["history"].append(f"[{today}] DOPPELGANGER active — something is impersonating {name} in the Undercity.")
                dead_npc["oracle_notes"] = (
                    dead_npc.get("oracle_notes", "") +
