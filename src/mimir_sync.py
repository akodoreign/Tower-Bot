...
from src.db_api import raw_query, raw_execute, get_npc_history, get_revealed_secrets
...
def _build_npc_notes(b: _NPCBridge, dj: dict) -> str:
    ...
    # History — now lives in npc_history table
    if b.bot_id:
        hist_rows = get_npc_history(b.bot_id, limit=30)
        if hist_rows:
            lines.append("\n## History")
            for hr in hist_rows:
                lines.append(f"- {hr['body']}")
    ...
    # Secrets — revealed list now in npc_revealed_secrets table
    secret = dj.get("secret", "") or ""
    revealed = get_revealed_secrets(b.bot_id) if b.bot_id else []
...
def _db_gear_candidates(slot: str, class_tags: list[str], level: int,
                         weapon_style: str = "any", n: int = 8) -> list[str]:
    """
    Pull n candidates from epic_gear_pool ordered by best fit first, randomised within tier.
    weapon_style only applies when slot='weapon'.
    """
    try:
        from src.db_api import raw_query
        tags_clause = " OR ".join(["class_tags = %s"] * len(class_tags))
        style_clause = ""
        style_params: tuple = ()
        if slot == "weapon" and weapon_style != "any":
            style_clause = "AND (weapon_style = %s OR weapon_style = 'any')"
            style_params = (weapon_style,)
        rows = raw_query(
            f"""SELECT name FROM epic_gear_pool
                WHERE slot = %s AND ({tags_clause}) AND min_level <= %s
                  AND enabled = 1 {style_clause}
                ORDER BY
                  CASE WHEN class_tags != 'any' THEN 0 ELSE 1 END,
                  RAND()
                LIMIT %s""",
            (slot, *class_tags, level, *style_params, n),
        ) or []
...
async def _infer_epic_npc_gear(bridge: _NPCBridge, base_gear: list) -> tuple[list, int, list[str]]:
    """
    Assign stat-appropriate epic gear from the epic_gear_pool DB table.
    One item per slot. Weapon style driven by STR vs DEX.
    ...
    # Class tag priority — most specific first, 'any' always included
    if "paladin" in cls_low:
        class_tags = ["paladin", "martial", "any"]
    elif "ranger" in cls_low:
        class_tags = ["ranger", "martial", "any"]
    elif "barbarian" in cls_low:
        class_tags = ["martial", "any"]
    elif martial:
        class_tags = ["martial", "any"]
    else:
        class_tags = ["caster", "any"]

    # Weapon style from actual stats — STR vs DEX
    scores = bridge.ability_scores or []
    str_score = scores[0] if len(scores) > 0 else 10
    dex_score = scores[1] if len(scores) > 1 else 10
    weapon_style = "finesse" if dex_score > str_score else "strength"

    async def add_slot(slot: str, fallbacks: list[str] | None = None,
                       style: str = "any") -> bool:
        candidates = _db_gear_candidates(slot, class_tags, level, weapon_style=style)
    ...
    if martial:
        # One weapon (stat-appropriate style), one armor, one shield, two wondrous
        await add_slot("weapon",  ["Sword of Sharpness", "Flame Tongue"], style=weapon_style)
        await add_slot("armor",   ["Armor, +3", "Dwarven Plate", "Dragon Scale Mail"])
        await add_slot("shield",  ["Shield, +3", "Sentinel Shield"])
        await add_slot("wondrous", ["Belt of Giant Strength (Hill)", "Ring of Protection"])
        await add_slot("wondrous", ["Cloak of Protection", "Cloak of Displacement",
                                    "Ioun Stone, Protection"])
        target_ac = 29 if level >= 17 else 25
    else:
        # Caster: focus, defensive layer, two wondrous
        focus = _spellcasting_focus_for_class(cls)
        if focus:
            await add_slot("focus", [
                focus.replace("+1 ", "+3 ", 1),
                focus.replace("+1 ", "+2 ", 1),
                focus,
            ])
        else:
            await add_slot("focus", ["Staff of Power", "Staff of the Magi",
                                     "Rod of Absorption"])
        await add_slot("wondrous", ["Robe of the Archmagi", "Bracers of Defense",
                                    "Cloak of Protection"])
        await add_slot("wondrous", ["Ring of Protection", "Amulet of Health"])
        await add_slot("wondrous", ["Ioun Stone, Protection", "Gem of Seeing",
                                    "Helm of Telepathy"])
...
def _build_world_overview() -> str:
    """World overview from MySQL gazetteer table."""
    try:
        ...
        if rows and rows[0].get("content_json"):
            ...
            gz = json.loads(cj) if isinstance(cj, str) else cj
            city = gz.get("city", {})
...
class MimirSyncEngine:
    ...
    async def sync_npc(self, npc_id: int) -> bool:
        ...
        # Extract data_json for notes doc — real columns take precedence for promoted fields
        dj: dict = {}
        if row.get("data_json"):
            try:
                dj = row["data_json"] if isinstance(row["data_json"], dict) else json.loads(row["data_json"])
                ...
                pass
        # Overlay real columns so Mimir gets the authoritative values
        for _col in ("rank", "motivation", "quote", "oracle_notes", "secret", "relationships"):
            if row.get(_col):
                dj[_col] = row[_col]
