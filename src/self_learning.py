...
async def _study_maps_today() -> Optional[str]:
    """
    Columbus reviews maps generated since yesterday midnight.

    Scans generated_modules/*/maps/ for PNG files written today, reconstructs
    what prompt/settings were used from the map filename + DB image_refs metadata,
    then runs ColumbusAgent on the full set. Findings go to src.log and journal.
    Consensus fixes (where Columbus identifies a clear actionable change) go to
    PATCHES.md in the same format the /patches command reads.
    """
...
async def run_learning_session(discord_client=None):
    ...
    studies = [
        ...
        # Columbus — reviews maps generated today for prompt/settings quality
        ("map_quality",        _study_maps_today,           "columbus_map_review"),
