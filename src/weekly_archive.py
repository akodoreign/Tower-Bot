...
def _archive_graveyard() -> int:
    ...
    try:
        from src.db_api import raw_query
        rows = raw_query(
            "SELECT * FROM npcs WHERE status = 'dead'"
        ) or []
        records = _db_rows_to_list(rows)
        absorbed = [r for r in records if r.get("data_json", {}) and
                    (r.get("tower_absorbed") or
                     (isinstance(r.get("data_json"), dict) and r["data_json"].get("tower_absorbed")))]
        if not absorbed:
            # Try parsing data_json string
            import json as _j
            for r in records:
                dj = r.get("data_json")
