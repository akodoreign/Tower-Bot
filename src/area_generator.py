...
def _gather_district_context(district: str) -> dict:
    ...
    npc_rows = raw_query(
        "SELECT name, faction, role, location, motivation FROM npcs"
    ...
    for nr in npc_rows:
        ctx["npcs"].append({
            "name":          nr["name"],
            "faction":       nr.get("faction") or "",
            "role":          nr.get("role") or "",
            "motivation":    (nr.get("motivation") or "")[:120],
            "location_note": (nr.get("location") or "")[:150],
        })
