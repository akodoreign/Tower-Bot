...
def register_bot_loop(loop: "asyncio.AbstractEventLoop") -> None:
    """Called once at bot startup so Flask can submit Mimir work to the correct loop."""
    global _bot_loop
    _bot_loop = loop


def get_bot_loop() -> "asyncio.AbstractEventLoop | None":
    """Return the bot's event loop if registered, else None."""
    return _bot_loop
