...
async def _pass5_generate_maps(
    ...
    """Read maps_manifest.json and copy matching maps from battle_maps_library."""
    import json as _json
    import shutil
    from src.db_api import raw_execute
    from src.mission_builder.vtt_renderer import write_grid_sidecar
    from src.battle_map_library import get_maps_for_mission

    manifest_path = out_dir / "maps_manifest.json"
    if not manifest_path.exists():
        logger.warning("[MAPS] No maps_manifest.json found ? skipping maps")
        return []
    ...
    for loc in map_entries[:6]:
        ...
        mission_hint = {
            ...
            "location": loc_name,
            "district": loc.get("district") or loc.get("area") or "",
            "area": loc.get("area") or loc.get("district") or loc_name,
            "mission_type": loc.get("mission_type") or loc_type,
            "type": loc.get("mission_type") or loc_type,
            "description": loc.get("description", ""),
            "tier": loc.get("tier", ""),
            "map_memory_key": f"{loc.get('district') or loc_name}:{loc_name}",
        }
        selected = get_maps_for_mission(mission_hint, count=1)
        if not selected:
            logger.info(f"[MAPS] No library map found for {loc_name!r}; skipping")
            continue
        row = selected[0]
        src_path = Path(row.get("file_path") or "")
        if not src_path.exists():
            logger.warning(f"[MAPS] Library map file missing for {loc_name!r}: {src_path}")
            continue
        safe_name = _safe_filename(loc_name, 30)
        suffix = src_path.suffix or ".png"
        map_file = maps_dir / f"{safe_name}_library_{row.get('id')}{suffix}"
        if not map_file.exists():
            shutil.copy2(src_path, map_file)
            write_grid_sidecar(map_file, mission_hint)
        ...
        raw_execute(
            ...
            "metadata_json=VALUES(metadata_json), ref_count=ref_count+1, updated_at=NOW()",
            (
                loc_name,
                str(map_file),
                _json.dumps({
                    "type": loc_type,
                    "mission_title": title,
                    "source": "battle_maps_library",
                    "battle_map_id": row.get("id"),
                    "source_file_path": row.get("file_path", ""),
                    "district": mission_hint["district"],
                    "mission_type": mission_hint["mission_type"],
                }),
            ),
        )
        logger.info(f"[MAPS] Reused library map #{row.get('id')}: {loc_name!r}")

    logger.info(f"[MAPS] Pass 5 complete ? {len(map_paths)} library maps copied")
    return map_paths
