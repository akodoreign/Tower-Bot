...
Keep it atmospheric and specific to the location. No JSON — just the description text."""
    ...
    return text or f"You arrive at {gather_location}. The area is busy but the items should be here somewhere."
...
async def _check_a1111() -> bool:
    for attempt in range(1, 11):
        try:
            ...
            pass
        if attempt < 10:
            logger.info(f"[GATHER] A1111 not ready (attempt {attempt}/10) — waiting 30s…")
            await asyncio.sleep(30)
    logger.warning("[GATHER] A1111 unavailable after 10 attempts")
    return False


# Keyed by keyword — checked in order, most specific first
_GATHER_LOCATION_STYLES: List[tuple] = [
    # ── Outdoor open spaces / points of interest ─────────────────────────────
    ("garden of accord",  "Town Exterior Table Map, sculpted hedgerows, stone pathways, central fountain, diplomatic meeting pavilion, ornamental trees, benches along paths"),
    ("pantheon walk",     "Town Exterior Table Map, wide ceremonial walkway, shrine alcoves along sides, temple facades flanking, offering braziers, open sky centre"),
    ("grand forum",       "Town Exterior Table Map, wide stone square, announcement podium at centre, radiating paths, public seating, political notice boards on pillars"),
    ("promenade",         "Town Exterior Table Map, wide walkway with railings, view platforms, neon signage on adjacent structures, luxury stalls, sky restaurant terraces"),
    ("sky platform",      "Town Exterior Table Map, elevated deck with railings, sky bridge connectors, luxury vendor stalls, open central space, dome visible above"),
    ("collapsed plaza",   "Town Exterior Table Map, shattered stone plaza, rubble piles and debris fields, scavenger camp structures, partial walls still standing, dangerous drop zones"),
    ("rubble field",      "Town Exterior Table Map, dense rubble scatter, collapsed building outlines visible, improvised paths through debris, hazard zones marked"),
    ("market square",     "Town Exterior Table Map, cobblestone square, permanent stall structures, central well or fountain, alleys radiating outward, crowd flow paths"),
    ("copper market",     "Town Exterior Table Map, trader stalls in rows, copper-toned awnings, narrow lanes between vendors, scales and crates as scatter"),
    ("night market",      "top-down night market map, lantern-lit stalls, shadowed alcoves between vendors, narrow passages, evening crowd flow paths"),
    ("food court",        "top-down outdoor food court map, cooking stall clusters, communal seating area, fire pit at centre, waste collection at edges"),
    ("spice alley",       "Town Exterior Table Map, spice vendor stalls packed tightly, hanging dried goods, barrels and sacks as scatter, single lane through"),
    ("cloth row",         "Town Exterior Table Map, fabric stall awnings overhanging lane, bales of cloth, measuring tables, colour-coded vendor sections"),
    ("neon strip",        "Town Exterior Table Map, wide central strip, tavern and theatre facades, arcade entrances, neon signage positions, crowd flow lanes"),
    ("the bridges",       "Town Exterior Table Map, multiple elevated walkways connecting platforms, rope bridge sections, guard posts at span entries, vertical drop zones"),
    ("the fringe",        "Town Exterior Table Map, last permanent structures, monitoring stations, open wasteland beginning at one edge, sparse building scatter"),
    ("the edge",          "Town Exterior Table Map, settlement ending abruptly, unknown territory beginning, minimal structures, warning markers"),
    # ── Gardens / parks / green spaces ───────────────────────────────────────
    ("garden",            "Town Exterior Table Map, cultivated plant beds in geometric patterns, winding gravel paths, stone benches, central water feature or fountain, trellis structures"),
    ("park",              "Town Exterior Table Map, open grass clearing, mature trees at edges, walking paths, rest areas, small pond, decorative planters"),
    ("green",             "Town Exterior Table Map, open lawn space, surrounding low buildings, community notice board, paths crossing at centre"),
    ("grove",             "Town Exterior Table Map, clustered trees in organic arrangement, root-covered ground, dappled clearing at centre, hidden alcoves between trunks"),
    # ── Plazas / squares / monuments ─────────────────────────────────────────
    ("plaza",             "Town Exterior Table Map, wide open stone square, central monument or fountain, radiating stone paths, flanking benches, decorative pillars"),
    ("square",            "Town Exterior Table Map, open cobblestone area, surrounding building facades, central landmark, vendor positions at edges"),
    ("forum",             "Town Exterior Table Map, large open meeting space, raised dais at one end, tiered seating along sides, public notice boards"),
    ("courtyard",         "top-down enclosed courtyard map, surrounded by building walls on three or four sides, central open space, well or fountain, gate entry"),
    ("memorial",          "Town Exterior Table Map, central monument or obelisk, radiating stone paths, name plaques along walls, solemn open space"),
    ("monument",          "Town Exterior Table Map, large central structure, cleared viewing space around it, low railings, tourist flow paths"),
    ("amphitheatre",      "Town Exterior Table Map, tiered semicircular seating, central performance stage, multiple entry archways, backstage area"),
    ("promenade",         "Town Exterior Table Map, long walkway with flanking trees or lights, viewing points, vendor alcoves, decorative paving"),
    # ── Markets / commercial outdoor ─────────────────────────────────────────
    ("bazaar",            "Town Exterior Table Map, sprawling vendor tent clusters, winding paths between stalls, central clearing, colour-coded merchant sections"),
    ("market",            "Town Exterior Table Map, dense vendor stalls, narrow lanes, open central square, crates and barrels as scatter, delivery alleys"),
    ("dock",              "Town Exterior Table Map, canal edge, moored boats, crane mechanisms, cargo netting, warehouse loading bays, narrow gangways"),
    # ── Streets / alleys ─────────────────────────────────────────────────────
    ("alley",             "Town Exterior Table Map, narrow winding passages, refuse alcoves, fire escapes, doorways, drainage gutters, dead-end alcove"),
    ("street",            "Town Exterior Table Map, cobblestone road, building facades along edges, lamp posts, manhole covers, vendor stalls"),
    ("row",               "Town Exterior Table Map, terraced building facades, narrow pavement, shop front entries, alley access between buildings"),
    ("strip",             "Town Exterior Table Map, wide lane, tavern and shop facades, signage positions, crowd flow markers"),
    # ── Underground / tunnels ────────────────────────────────────────────────
    ("sewer",             "Dungeon Interior Table Map, brick tunnels, water channels, stone walkways, iron grates, pipes and drains, maintenance catwalks"),
    ("undercroft",        "Dungeon Interior Table Map, low vaulted ceiling suggested by arch lines, storage alcoves, brick pillars, hidden passages"),
    ("tunnel",            "Dungeon Interior Table Map, narrow stone passage, branching routes, support beam positions, debris scatter"),
    ("cave",              "Dungeon Interior Table Map, organic rock walls, stalactite field edges, underground pool, rope bridges, luminescent mushroom clusters"),
    ("depths",            "Dungeon Interior Table Map, cavern space, rough stone walls, multiple passage exits, natural columns, pooled water"),
    ("warrens",           "Dungeon Interior Table Map, improvised tunnel network, reclaimed rooms, salvaged material walls, dead ends, hidden chambers"),
    # ── Interior / buildings ─────────────────────────────────────────────────
    ("archive",           "Interior Table Map, tall shelving rows, reading tables, card catalogue stations, archive alcoves, single main entrance"),
    ("vault",             "Interior Table Map, heavy reinforced entry, narrow access corridor, security alcoves, safety deposit rows"),
    ("laboratory",        "Interior Table Map, experiment tables, reagent shelves, distillation apparatus, specimen jars, containment zone"),
    ("guild hall",        "Interior Table Map, main hall, notice board wall, meeting table, trophy displays, administrative desk alcoves"),
    ("guild",             "Interior Table Map, guild space, open working area, member stations, storage alcoves, entry checkpoint"),
    ("scriptorium",       "Interior Table Map, writing desk rows, ink and scroll storage, illumination stations, quiet alcoves, librarian desk"),
    ("warehouse",         "Interior Table Map, large storage crates in rows, loading dock, support columns, forklift paths, staging area"),
    ("foundry",           "Interior Table Map, central forge structures, cooling troughs, material bins, safety exclusion zones, heavy machinery"),
    ("barracks",          "Interior Table Map, bunk bed rows, weapon racks, armour stands, training mat, officer desk, mess table"),
    ("hall",              "Interior Table Map, long central space, flanking alcoves or rooms, entrance at one end, stage or dais at other"),
    ("ruin",              "Town Exterior Table Map, collapsed walls, rubble piles, partial flooring, exposed foundations, overgrown reclamation, debris scatter"),
]
...
async def _generate_gather_map(
    ...
    import re
    map_style = _gather_map_style(gather_location)
    ...
    faction_tint = {
        "Wardens of Ash":   "grey stone utilitarian aesthetic, practical district",
        "Patchwork Saints": "community district, mismatched colourful signs, lived-in",
        "Wizards Tower":    "arcane district, glowing rune markers, restricted zone signage",
        "Argent Blades":    "professional district, mercenary quarter, weapons visible in shops",
        "Obsidian Lotus":   "shadow district, minimal signage, concealed entry points",
        "Glass Sigil":      "wealth district, elegant facades, clean streets",
        "Serpent Choir":    "hidden district, cult markings barely visible, dim lighting",
        "Iron Fang Consortium": "industrial district, warehouse row, heavy machinery marks",
    }.get(faction, "neutral city district")
    ...
    positive = ", ".join(filter(None, [
        ...
        "multiple entry and exit paths, scatter objects, points of interest clearly marked",
    ]))
    ...
    for map_attempt in range(1, max_map_attempts + 1):
        try:
            ...
            decision = await wait_for_a1111_turn("gather_map", max_wait_seconds=60)
            if not decision.run_now:
                logger.info(f"[GATHER] A1111 deferred by resource cop: {decision.reason}")
                from src.mission_builder.vtt_renderer import save_vtt_battlemap
                save_vtt_battlemap(out, None, context={"title": gather_location, "location": gather_location, "prompt": positive, "kind": "gather exploration"})
                return out
            ...
            from src.mission_builder.vtt_renderer import save_vtt_battlemap
            save_vtt_battlemap(out, images[0], context={"title": gather_location, "location": gather_location, "prompt": positive, "kind": "gather exploration"})
            logger.info(f"[GATHER] Map saved: {out.name}")
            return out
        except Exception as e:
            logger.warning(f"[GATHER] Map attempt {map_attempt}/{max_map_attempts} failed: {e}")
            if map_attempt < max_map_attempts:
                wait = min(30 * map_attempt, 120)
                logger.info(f"[GATHER] Retrying map in {wait}s…")
                await asyncio.sleep(wait)

    logger.error("[GATHER] Map generation failed after 10 attempts — using deterministic fallback")
    from src.mission_builder.vtt_renderer import save_vtt_battlemap
    save_vtt_battlemap(out, None, context={"title": gather_location, "location": gather_location, "prompt": positive, "kind": "gather exploration"})
    logger.info(f"[GATHER] Deterministic VTT map saved: {out.name}")
    return out
...
def _skill_check_card(item_data: Dict, check_num: int, encounters_between: List[Dict]) -> str:
...
def render_gather_module(
    ...
    for i in range(1, stock + 1):
        ...
        check_cards += _skill_check_card(item_data, i, between)
...
async def build_gather_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    if str(os.getenv("MODULE_GENERATE_MAPS", "false")).lower() in ("1", "true", "yes", "on"):
        if await _check_a1111():
            ...
            logger.warning("[GATHER] A1111 not available — skipping map")
