...
def _pick_adventurer_party(strength: Dict[str, Any]) -> Optional[Dict]:
    rows = _db_rows(
        "SELECT party_name, tier, reputation, status, profile_json, members_json "
    ...
    return {
        ...
        "tier": row.get("tier") or profile.get("tier") or "Unknown",
...
{{
  ...
  "target_spotted_read_aloud": "2 sentences, present tense — the exact words the DM reads the moment the target (or their convoy) enters the ambush zone. Sensory and specific."
    ...
    if data:
        return data
    return {
        "contact_speech": (lambda: __import__('src.mission_builder.read_aloud_library', fromlist=['scene_read_aloud']).scene_read_aloud("s1", mission_type="bounty", contact=roles.get("hiring","the contact"), faction=roles.get("hiring",""), location=location.get("name","")))(),
        ...
        "target_spotted_read_aloud": (lambda: __import__('src.mission_builder.read_aloud_library', fromlist=['scene_read_aloud']).scene_read_aloud("s3", mission_type="escort", contact=roles.get("hiring","the contact"), location=location.get("name","")))(),
    }
...
async def _generate_map(map_plan: Dict, location: Dict, roles: Dict[str, str], out_dir: Path) -> Optional[Path]:
    ...
    return copy_library_map_for_mission(
        {"faction": roles.get("hiring", "")},
        out,
        mission_type="ambush",
        location_name=location.get("name", ""),
        district=location.get("district", ""),
        description=f"{map_plan.get('name', '')} {map_plan.get('prompt', '')} {location.get('type', '')}",
        map_type="sewer" if "sewer" in str(map_plan.get("name", "")).lower() else "",
    )
...
def render_ambush_module(
    ...
    body += _card("Faction Roles",
        ...
        f'<strong>Interference:</strong> {_e(roles.get("interference") or "No third party")}</p>', "#555")
    body += _card("Target",
...
async def build_ambush_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    # DB-monster guards (role "CR <n> <type>") get real stat blocks in Mimir/DDB;
    # named faction NPCs are people, not catalog monsters, so those are skipped.
    _g_enemies = []
    for _gm in (guard.get("members") or []):
        _grole = str(_gm.get("role", ""))
        if _grole.startswith("CR ") and _gm.get("name"):
            _gparts = _grole.split()
            _g_enemies.append({
                "name": _gm["name"],
                "cr": _gparts[1] if len(_gparts) > 1 else "1",
                "count": 1,
                "notes": f"{roles['guarding']} guard - {_gm.get('note', '')}"[:180],
                "creature_type": " ".join(_gparts[2:]) or "humanoid",
            })
    _enriched = await _me(_mimir_id or "", _g_enemies) if _g_enemies else []
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="AMBUSH")
    except Exception as e:
