...
async def _ollama(prompt: str, system: str = "", tokens: int = 2200) -> str:
    ...
    payload = {
        ...
        "messages": messages,
        "stream": False,
        "options": {"temperature": 0.78, "num_predict": tokens, "num_ctx": _fit_ctx(system + prompt, tokens)},
    }
    try:
...
def _research_table(routes: List[Dict[str, str]]) -> str:
    rows = []
    for r in routes:
        setup   = r.get("setup") or f"Spend {r.get('time', 'time')} working {r.get('site', 'this lead')} via {r.get('skill', 'the skill')}."
        success = r.get("clue", "")
        failure = r.get("failure") or "the lead dead-ends or costs extra time; the clue must come from another route."
        rows.append((
            r["site"],
            f"{r.get('skill', '')} ({r.get('time', '')}) | Setup: {setup} | "
            f"On Success: clue -- {success} | On Failure: {failure}",
        ))
    return _table(rows)
...
def render_puzzle_module(mission: dict, sponsor: str, puzzle_type: str, plan: Dict[str, Any], research_routes: List[Dict[str, str]], strength: Dict[str, Any], calendar: List[str]) -> str:
    ...
    return _page(title, body, sponsor, mission_id=mission.get("id"))
...
async def build_puzzle_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    # Pull a puzzle-site map from the library
    _pz_maps: list = []
    try:
        ...
        _lib = copy_library_map_for_mission(mission, out_dir / "maps" / "puzzle_site.png",
                                            mission_type="puzzle", map_type="interior")
        if _lib:
            _pz_maps = [_lib]
            (out_dir / "maps.html").write_text(
                render_maps_page(title, sponsor, _pz_maps, "{}"), encoding="utf-8"
            )
    except Exception as _me:
        logger.warning(f"[PUZZLE] Library map failed: {_me}")

    box_components = component_links(has_maps=bool(_pz_maps))
    if _pz_maps:
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="PUZZLE")
    except Exception as e:
