...
async def _generate_room_llm(
    ...
    try:
        ...
        decision = await wait_for_ollama_turn("dungeon_room", track="primary", max_wait_seconds=60)
        if not decision.run_now:
            logger.info(f"🏰 Ollama busy, deferring room {room.room_id}: {decision.reason}")
            return None
