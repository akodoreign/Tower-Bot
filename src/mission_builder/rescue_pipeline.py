...
def _needs_map(subtype: str) -> bool:
    return _mode(subtype) == "active"


async def _ollama(prompt: str, tokens: int = 1200) -> str:
    payload = {"model": OLLAMA_MODEL, "messages": [{"role": "user", "content": prompt}], "stream": False, "options": {"temperature": 0.85, "num_predict": tokens}}
    try:
...
async def _check_a1111() -> bool:
    for attempt in range(1, 11):
        try:
            ...
            pass
        if attempt < 10:
            logger.info(f"[RESCUE] A1111 not ready (attempt {attempt}/10) — waiting 30s…")
            await asyncio.sleep(30)
    logger.warning("[RESCUE] A1111 unavailable after 10 attempts")
    return False


_RESCUE_MAP_STYLE = {
    "captive_extraction":   "Interior Objects Table Map, captive holding area, locked rooms, guard posts, extraction corridor, chokepoint entry",
    "hostage_negotiation":  "Interior Objects Table Map, negotiation room floor plan, open central space, multiple exits, cover positions, tension chokepoints",
    "disaster_area":        "Town Exterior Objects Table Map, disaster site, collapsed structures, rubble fields, hazard zones, rescue access paths, debris scatter",
    "cleanup_rescue":       "Town Exterior Objects Table Map, aftermath site, collapsed buildings, debris, survivor shelter areas, safe paths marked",
    "medical_extraction":   "Interior Objects Table Map, medical extraction site, treatment area, narrow corridors, emergency exits, stretcher routes",
    "prison_break":         "Dungeon Interior Objects Table Map, prison cell block, locked cell doors, guard post, central walkway, emergency exits, armory alcove",
    "cursed_ritual":        "Dungeon Interior Objects Table Map, ritual chamber, arcane circle on floor, altar, collapsed pillars, containment wards, dark passages",
    "battlefield_pullout":  "Big Town Exterior Objects Table Map, active battlefield, extraction route, cover positions, fallen fighters, rally point, exit path",
    "missing_person":       "Town Exterior Objects Table Map, search area, district streets, alley entrances, last known locations marked, scatter objects",
}

_RESCUE_DUNGEON_SUBTYPES = {"captive_extraction", "hostage_negotiation", "medical_extraction", "prison_break", "cursed_ritual"}

async def _generate_map(subtype: str, location: Dict[str, Any], out_dir: Path) -> Optional[Path]:
    out = out_dir / "rescue_map.png"
    import re
    _raw_style = _RESCUE_MAP_STYLE.get(subtype, "rescue site, approach routes, extraction exit, cover positions")
    is_dungeon = subtype in _RESCUE_DUNGEON_SUBTYPES or "dungeon" in _raw_style.lower()
    ...
    prompt = ", ".join(filter(None, [
        ...
        triggers,
        _style,
        f"{location.get('name')} {location.get('district')}",
        loc_desc,
    ...
    _vtt_context = {"title": location.get("name"), "location": location, "prompt": prompt, "kind": "rescue interior fort"}
    for map_attempt in range(1, 11):
        try:
            from src.resource_cop import wait_for_a1111_turn
            decision = await wait_for_a1111_turn("rescue_map", max_wait_seconds=60)
            if not decision.run_now:
                logger.info(f"[RESCUE] A1111 deferred by resource cop: {decision.reason}")
                save_vtt_battlemap(out, None, context=_vtt_context)
                return out
            ...
            save_vtt_battlemap(out, images[0], context=_vtt_context)
            return out
        except Exception as e:
            logger.warning(f"[RESCUE] Map generation attempt {map_attempt}/10 failed: {e}")
            if map_attempt < 10:
                _backoff = min(30 * map_attempt, 120)
                logger.info(f"[RESCUE] Retrying map in {_backoff}s…")
                await asyncio.sleep(_backoff)
    logger.error("[RESCUE] Map generation failed after 10 attempts — using deterministic fallback")
    save_vtt_battlemap(out, None, context=_vtt_context)
    return out
...
async def build_rescue_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    map_path = None
    if _needs_map(subtype) and str(os.getenv("MODULE_GENERATE_MAPS", "false")).lower() in ("1", "true", "yes", "on"):
        if await _check_a1111():
            map_path = await _generate_map(subtype, location, out_dir)
        else:
            logger.warning("[RESCUE] A1111 not available - skipping map")
