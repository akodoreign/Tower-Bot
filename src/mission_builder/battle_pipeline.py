...
# Distinct, mechanically varied hazards used to steer the local model and to
# dedupe repeats across pocket fights. Local to the battle pipeline by design.
BATTLE_FALLBACK_HAZARDS = [
    "Tilted rubble turns half the zone into difficult terrain.",
    "Ruptured conduits vent scalding steam in a 10-ft line each round (DC 13 Dex save or 1d6 fire).",
    "Flickering light leaves the far half lightly obscured until a light source is planted.",
    "A sagging upper floor drops debris if three or more combatants cluster (DC 14 Dex save).",
    "A rift seam in the floor drags Medium-or-smaller creatures 5 ft toward it (DC 12 Str save).",
    "Snapped cabling whips live current across one chokepoint (DC 13 Dex save or 1d8 lightning).",
]


def _cr_sort_key(m: Dict) -> float:
    """Numeric CR for ordering; tolerates ints, decimals, and '1/2' style."""
    raw = str(m.get("cr") if isinstance(m, dict) else m or "0").strip()
    try:
        if "/" in raw:
            n, d = raw.split("/")
...
def _pick_void_creatures(target_cr: int, count: int) -> List[Dict]:
    ...
    if rows:
        # Order ascending so the boss CR escalates across fights instead of
        # bouncing (e.g. 13/13/9). Grunt counts already ramp per fight.
        rows = sorted(rows, key=_cr_sort_key)
        return [
...
def _battle_mimir_enemies(
    ...
    for fight in pocket_fights:
        ...
        if fight.get("db_enemy"):
            # In void / breakout fights the DB enemy is a single statted boss,
            # not N grunts; show it as x1 so the reference is not "boss x14".
            boss_count = 1 if conflict_type in ("faction_vs_void", "monster_breakout") else count
            entry = mission_enemy_entry(fight["db_enemy"], count=boss_count, notes=note)
...
async def _ollama(prompt: str, tokens: int = 1400) -> str:
    ...
    payload = {
        ...
        "stream":   False,
        "think":    False,
        "options":  {"temperature": 0.84, "num_predict": tokens, "num_ctx": _fit_ctx(prompt, tokens)},
    }
...
async def _generate_pocket_fight(
    ...
    # Per-fight zone + hazard variety so the pocket fights do not all read as
    # "the central plaza" with the same "unstable ground" hazard. Steers the
    # small local model toward a distinct zone/hazard each call. Local to this
    # pipeline by design (project rule: pipelines stay independent).
    _zones = ["the outer approach", "the breached threshold", "the central hold",
              "the inner sanctum", "the collapsed rear gallery", "the high gantries"]
    zone = _zones[fight_index % len(_zones)]
    fb_hazard = BATTLE_FALLBACK_HAZARDS[fight_index % len(BATTLE_FALLBACK_HAZARDS)]
...
LOCATION: {zone} of {location['name']}, {location['district']}
THIS IS FIGHT {fight_index + 1} of the battle and MUST take place in a different part of the site than the other fights.
Suggested hazard angle for this zone (rephrase in your own words, do not copy verbatim): {fb_hazard}
...
{{
  ...
  "outcome_if_lost": "one sentence — what changes if the party loses or retreats"
    ...
    return {
        ...
        "read_aloud":           data.get("read_aloud", f"The fight pushes into {zone} of {location['name']}. {enemy_name} hold the ground ahead, and there are more of them than you would like."),
        "setting":              data.get("setting", f"A clash in {zone} of {location['name']}."),
        ...
        "hazard":               data.get("environmental_hazard") or fb_hazard,
...
async def _generate_battle_map(
    ...
    return copy_library_map_for_mission(
        ...
        description=f"{conflict_type} {(location.get('description') or '')[:150]}",
        map_type="street",
    )
...
async def build_battle_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    # Dedupe near-identical hazards across fights (the small local model often
    # echoes the same line); swap repeats for a distinct, unused hazard.
    _seen_haz: set = set()
    for _f in pocket_fights:
        _h = (_f.get("hazard") or "").strip()
        _key = _h.lower()[:40]
        if not _h or _key in _seen_haz:
            for _cand in BATTLE_FALLBACK_HAZARDS:
                if _cand.lower()[:40] not in _seen_haz:
                    _f["hazard"] = _cand
                    _key = _cand.lower()[:40]
                    break
        _seen_haz.add(_key)
    ...
    if not _mimir_id:
        try:
            ...
            logger.warning(f"[DDB_HB] DDB direct push failed: {_dex}")
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="BATTLE")
    except Exception as _e:
