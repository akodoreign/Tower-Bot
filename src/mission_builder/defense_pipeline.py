  ...
  "arrival_read_aloud": "2-3 sentences, present tense, sensory — what the party sees when they first arrive at the position to defend (the people already there, the sounds of preparation, the sense of what is at stake)"
    ...
    if not data:
        data = {
            ...
            "arrival_read_aloud": (lambda: __import__('src.mission_builder.read_aloud_library', fromlist=['scene_read_aloud']).scene_read_aloud("s1", mission_type="defense", contact="the defender", location=location))(),
        }
    return data
...
async def _generate_defense_map(
    ...
    """Select one defensible tactical map from the battle map library."""
    ...
    return copy_library_map_for_mission(
        ...
        description=" ".join(str(v) for v in defenses.values())[:500],
    )
...
def _intel_lead_cards(leads: List[Dict]) -> str:
    ...
    for lead in leads:
        ...
        cards += (
            ...
            + f'<div style="margin-top:2px;font-size:12px;color:#7b1e1e;">On failure: {_e(lead.get("failure") or "the prep time is spent without finishing the upgrade -- no modifier gained, and the crew is committed when the attack lands.")}</div>'
            + f'</div>'
        )
    return cards
...
async def build_defense_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="DEFENSE")
    except Exception as _e:
