...
def _make_output_dir(title: str, mission_id: int | None = None) -> Path:
    ts  = datetime.now().strftime('%Y%m%d_%H%M%S_%f')[:19]  # include microseconds
    mid = f"_id{mission_id}" if mission_id else ""
    out = OUTPUT_BASE / f"{_safe_filename(title)}{mid}_{ts}"
...
def _pick_opposing_faction(hiring_faction: str) -> str:
    """
    Return the most narratively credible opposing faction using DB data.
    Priority: declared rival → hostile/negative rep → any different faction.
    """
    try:
        from src.db_api import raw_query

        # 1. Declared rivals in faction_affiliations
        if hiring_faction:
            rows = raw_query(
            ...
            if rows and rows[0].get("guild_name"):
                return rows[0]["guild_name"]

        # 2. Hostile-tier or negative-rep factions
        rows = raw_query(
            "SELECT faction_name FROM faction_reputation "
            "WHERE (tier IN ('Hostile','Enemy') OR reputation_score < -5) "
            "AND faction_name NOT LIKE %s "
            "AND faction_name NOT LIKE '%[%' AND faction_name != 'Unknown' "
            "ORDER BY reputation_score ASC, RAND() LIMIT 1",
            (f"%{hiring_faction}%" if hiring_faction else "%%",),
        ) or []
        if rows and rows[0].get("faction_name"):
            return rows[0]["faction_name"]

        # 3. Any other faction
        rows = raw_query(
            "SELECT faction_name FROM faction_reputation "
            "WHERE faction_name != %s AND faction_name NOT LIKE '%[%' "
            "ORDER BY RAND() LIMIT 1",
            (hiring_faction or "",),
        ) or []
        if rows and rows[0].get("faction_name"):
...
def _mission_seed(mission: dict) -> dict:
    ...
    return {
        ...
        "opposing_faction": _clean(mission.get("opposing_faction") or _pick_opposing_faction(faction)),
    }
...
def _fit_ctx(prompt: str, reply_tokens: int) -> int:
    """Size the Ollama context window to the prompt + planned reply. With a
    fixed num_ctx, Ollama silently truncates either an oversized prompt or the
    long reply (yielding empty/unparseable output -> deterministic scaffold).
    Only raises; capped at 32k. Local to this pipeline by the no-shared-helpers
    rule."""
        ...
        data = await call_ollama(
            {
                ...
                "options": {
                    ...
                    "num_ctx": _fit_ctx(prompt, int(os.getenv("MODULE_BLUEPRINT_TOKENS", "1600"))),
                    "temperature": 0.75,
                    "think": False,
                },
            },
...
def _scene_data(ctx: dict) -> list[dict]:
    ...
    # ── Scene 1 read-aloud: 10+ variants per type, DB-fed ──────────
    from src.mission_builder.read_aloud_library import scene_read_aloud as _ra
    def _s1_read() -> str:
        return _ra("s1", mission_type=mtype, contact=contact,
                   faction=ctx.get("faction",""), title=ctx.get("title",""),
                   location=sites.get("contract",""))
    # ── Scene 3 read-aloud: 10+ variants per type, DB-fed ──────────
    def _s3_read() -> str:
        return _ra("s3", mission_type=mtype, contact=contact,
                   faction=ctx.get("faction",""), title=ctx.get("title",""),
                   location=sites.get("route",""))
    ...
    return [
        ...
        },
        {
            ...
            "read": _ra("s2", mission_type=mtype, contact=contact,
                        faction=ctx.get("faction",""), title=title,
                        location=sites.get("leads",""),
                        extra={"leads": sites.get("leads","")}),
            "notes": [
...
def _loot_table_markdown(ctx: dict) -> str:
    try:
        from src.treasure import roll_treasure, format_treasure_for_discord
        pkg = roll_treasure(cr=ctx.get("cr", 5), tier=ctx.get("tier", "standard"))
        ctx["_treasure_pkg"] = pkg
        ec = pkg["ec"]
        lines = [f"**{ec:,} EC** (contract reward)"]
        for item in pkg.get("items", []):
            ch = f" ({item['charges']})" if item.get("charges") and item["charges"] != "Permanent" else ""
            lines.append(f"- {item['name']}{ch} — {item.get('effect','')}")
        for gem in pkg.get("gems", []):
            lines.append(f"- {gem['name']} — {gem.get('effect','')}")
        if pkg.get("magic"):
            m = pkg["magic"]
            lines.append(f"- {m['name']} ({m.get('rarity','').title()} {m.get('item_type','')}) — {m.get('effect','')}")
        if pkg.get("mimir"):
            m = pkg["mimir"]
            lines.append(f"- {m['name']} ({m.get('rarity','').title()}, from Mimir) — {m.get('effect','')}")
        return "## Loot\n" + "\n".join(lines)
    except Exception as e:
        return f"## Loot\n- {ctx.get('reward', 'Contract reward as agreed.')}"
...
def _map_manifest(ctx: dict, scenes: list[dict]) -> list[dict]:
    ...
    for scene in scenes[1:5]:
        ...
        entries.append({
            ...
            "district":     ctx.get("primary_location", ""),
            "area":         ctx.get("primary_location", ""),
...
async def generate_published_module(mission: dict, player_name: str = "") -> Optional[Path]:
    ...
    out_dir = _make_output_dir(ctx["title"], mission_id=int(_mid) if _mid else None)
    ...
    # NPC knowledge cards — one card per named NPC across all scenes
    try:
        from src.mission_builder.npc_knowledge_cards import build_npc_cards_html
        all_npc_lines: list[str] = []
        for sc in scenes:
            all_npc_lines.extend(sc.get("npcs", []))
        extra = [n for n in [ctx.get("contact"), ctx.get("enemy")] if n]
        cards_html = build_npc_cards_html(all_npc_lines, ctx, extra_names=extra)
        if cards_html:
            _npc_page_html = render_component(
                "NPC Reference",
                ctx["title"],
                "",
                ctx["faction"],
            ).replace("</body>", cards_html + "</body>")
            (out_dir / "npcs.html").write_text(_npc_page_html, encoding="utf-8")
            component_links.append(("npcs", "npcs.html"))
            logger.info(f"[PUBLISHED] NPC knowledge cards written")
    except Exception as _e:
        logger.warning(f"[PUBLISHED] NPC knowledge cards failed: {_e}")
    ...
    # If no generated maps, pull one from battle_maps_library (always available)
    if not map_paths:
        try:
            from src.battle_map_library import copy_library_map_for_mission
            district = ctx.get("district") or ""
            mtype    = (ctx.get("mission_type") or ctx.get("type") or "").lower()
            lib_paths = copy_library_map_for_mission(out_dir, district_slug=district, mission_type=mtype)
            if lib_paths:
                map_paths = lib_paths
                logger.info(f"[PUBLISHED] Library map copied: {[p.name for p in lib_paths]}")
        except Exception as e:
            logger.warning(f"[PUBLISHED] Library map copy failed: {e}")
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="PUBLISHED")
