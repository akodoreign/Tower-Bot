        ...
        async with httpx.AsyncClient(timeout=240.0) as c:
            r = await c.post(
                ...
                json={
                    ...
                    "stream": False,
                    "options": {"temperature": 0.88, "num_predict": tokens, "num_ctx": _fit_ctx(prompt, tokens)},
                },
            )
            r.raise_for_status()
...
JSON only."""
    ...
    for attempt in range(3):
        ...
        if data:
            break
        logger.warning(f"[STRANGE] LLM returned no usable plan (attempt {attempt + 1}/3) for '{mission.get('title', 'Strange Occurrence')}'")
        await asyncio.sleep(2)
    if not data:
        logger.warning("[STRANGE] LLM unavailable after retries; using mission-faithful fallback")
...
def _evidence_table(rows: List[Dict[str, Any]]) -> str:
    def _setup(r: Dict[str, Any]) -> str:
        return r.get("setup") or f"Work the {r.get('lane', 'lane')} with {r.get('skill', 'the relevant skill')} to surface what it hides."
    body = "".join(
        f"<tr><td>{_e(r.get('lane'))}</td><td>{_e(r.get('skill'))} DC {_e(r.get('dc'))}</td>"
        f"<td>{_e(_setup(r))}</td><td>{_e(r.get('clue'))}</td><td>{_e(r.get('if_failed'))}</td></tr>"
        for r in rows
    )
    return "<table><tr><th>Lane</th><th>Skill</th><th>Setup</th><th>On Success (Clue)</th><th>On Failure</th></tr>" + body + "</table>"
...
def render_module(mission: dict, occurrence_type: str, sponsor: Dict[str, str], locations: List[Dict[str, Any]], parties: List[Dict[str, str]], plan: Dict[str, Any], dcs: Dict[str, int], strength: Dict[str, Any]) -> str:
    ...
    _ret = (mission.get("returned_npc_name") or "").strip()
    if _ret:
        _rstatus = mission.get("returned_npc_status") or "undead"
        _rhints = mission.get("returned_npc_hints") or []
        body += _card(
            "DM Secret - The Illegal Return",
            f"<p>The strange occurrence is <strong>{_e(_ret)}</strong>, who has returned from the graveyard "
            f"(<em>{_e(_rstatus)}</em>) without leave. Players should <strong>deduce</strong> this from the clues -- "
            f"do not hand them the name.</p>"
            f"<h3>Clues planted (no name on the card)</h3>{_ul(_rhints)}"
            f"<p><strong>Resolution:</strong> success lays {_e(_ret)} back to rest -- the board returns them to the "
            f"graveyard automatically on completion, whether a player party or an NPC party clears it.</p>",
            "#7b1e1e",
        )
...
def render_session(mission: dict, sponsor: Dict[str, str], plan: Dict[str, Any]) -> str:
    ...
    # Scene order from the plan's structure string → ordered checklist
    structure_str = plan.get("structure") or "Report -> Scene -> Truth -> Resolution"
    scene_steps = [s.strip() for s in re.split(r"->|→|\|", structure_str) if s.strip()]
    scene_html = "".join(
        f'<p><input type="checkbox"> <strong>Scene {i}:</strong> {_e(s)}</p>'
        for i, s in enumerate(scene_steps, 1)
    ) or "<p>No scene order defined.</p>"

    # Explanation ladder → ordered progress track
    ladder = plan.get("explanation_ladder") or []
    ladder_html = "".join(
        f'<p><input type="checkbox"> {_e(step)}</p>' for step in ladder
    ) or "<p>No explanation ladder defined.</p>"

    # Witnesses checklist
    witnesses_html = "".join(
        f'<p><input type="checkbox"> <strong>{_e(w.get("name","?"))}</strong> — {_e(w.get("role",""))}</p>'
        for w in plan.get("witnesses", [])
    ) or "<p>No witnesses listed.</p>"

    # Evidence checklist
    evidence_html = "".join(
        f'<p><input type="checkbox"> {_e(e.get("lane",""))}: {_e(e.get("clue",""))}</p>'
        for e in plan.get("evidence_ladder", [])
    ) or "<p>No evidence lanes defined.</p>"

    # When the party stalls — quick escalation menu
    stall_html = (
        "<p><strong>Choose one escalation if momentum stalls:</strong></p>"
        "<p><input type='checkbox'> A witness recants or disappears</p>"
        "<p><input type='checkbox'> Faction arrives and demands the party stand down</p>"
        "<p><input type='checkbox'> The occurrence does something the public sees</p>"
        "<p><input type='checkbox'> A new record surfaces contradicting the current theory</p>"
        "<p><input type='checkbox'> TNN runs a story that makes the party look reckless</p>"
        "<p><input type='checkbox'> The uncanny rule triggers visibly in a crowded space</p>"
    )

    body = f'<h1 style="color:{fc};">{_e(title)}</h1>'
    body += _card("Scene Order", scene_html, fc)
    body += _card("Explanation Progress", ladder_html, "#3a6898")
    body += _card("Witnesses", witnesses_html, "#8a5a1f")
    body += _card("Evidence Checklist", evidence_html, "#3a6898")
    body += _card("Case State", _table([
        ("Sponsor", faction),
        ("Contact", sponsor["contact"]),
        ("Uncanny rule", plan.get("uncanny_rule") or "TBD"),
    ...
    body += _card("When The Party Stalls", stall_html, "#7b1e1e")
    body += _card("Resolution", (
        "<p><strong>Chosen ending:</strong> <select>"
        "<option>Contract Clean</option><option>Moral Override</option>"
        "<option>Protected Weird</option><option>Culprit Exposed</option>"
        "<option>Proper Rest</option><option>Public Scandal</option>"
        "<option>Bad Ending</option></select></p>"
        "<p><input type='checkbox'> Occurrence contained &nbsp;"
        "<input type='checkbox'> Truth revealed &nbsp;"
        "<input type='checkbox'> Faction satisfied</p>"
        "<p><strong>Pay earned?</strong> <select>"
        "<option>Full</option><option>Partial</option>"
        "<option>No EC, Kharma/standing only</option><option>No reward, fallout</option></select></p>"
        "<textarea rows='4' style='width:100%;font-family:inherit;' placeholder='What did the party decide and who is angry?'></textarea>"
    ), "#2a6a2a")
    return _page(title, body, faction, mission_id=mission.get("id"))
...
async def build_strange_occurrences_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    _html = render_module(mission, occurrence_type, sponsor, locations, parties, plan, dcs, strength)
    ...
    # Pull an incident-site map from the library
    _so_maps: list = []
    try:
        ...
        _lib = copy_library_map_for_mission(mission, out_dir / "maps" / "incident_site.png",
                                            mission_type="investigation", map_type="interior", district=_district)
        if _lib:
            _so_maps = [_lib]
            (out_dir / "maps.html").write_text(
                render_maps_page(title, sponsor["sponsor"], _so_maps, "{}"), encoding="utf-8"
            )
    except Exception as _me:
        logger.warning(f"[STRANGE] Library map failed: {_me}")

    try:
        from src.mission_builder.html_renderer import render_index
        _so_clinks = component_links(has_maps=bool(_so_maps))
        if _so_maps:
            _so_clinks.append(("maps", "maps.html"))
        index_html = render_index(
            ...
            chapters=[],
            components=_so_clinks,
            chart_pack=None,
            map_count=len(_so_maps),
        )
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="STRANGE")
    except Exception as e:
