...
def _npc_seeds(mission: dict, context: Dict[str, Any], rival_party: Dict[str, Any]) -> Dict[str, Any]:
    """
    Pull real NPC voice data from DB for the key roles in this recovery.
    Returns a dict keyed by role label; each entry has name, quote, motivation, rank, faction.
    Used to ground LLM dialogue and personalize the fallback bank.
    """
    seeds: Dict[str, Any] = {}
    district = context.get("district", "")

    # Sponsor contact — try mission.contact, then faction-affiliated NPC
    contact_name = mission.get("contact") or mission.get("npc_giver") or ""
    if contact_name:
        rows = _db_rows(
            "SELECT name, `rank`, motivation, quote, oracle_notes, faction, role FROM npcs "
        ...
        if rows:
            ...
            seeds["sponsor"] = {
                "name": r["name"],
                "rank": r.get("rank") or "",
                "motivation": (r.get("motivation") or "")[:180],
                "quote": r.get("quote") or "",
                "faction": r.get("faction") or mission.get("faction", ""),
            }
    if "sponsor" not in seeds:
        # Fall back to any NPC affiliated with the hiring faction
        faction = mission.get("faction", "")
        if faction:
            rows = _db_rows(
                "SELECT name, `rank`, motivation, quote, faction, role FROM npcs "
                "WHERE faction LIKE %s AND status NOT IN ('dead','retired') ORDER BY RAND() LIMIT 1",
                (f"%{faction}%",),
            )
            if rows:
                r = rows[0]
                seeds["sponsor"] = {
                    ...
                    "motivation": (r.get("motivation") or "")[:180],
                    "quote": r.get("quote") or "",
                    "faction": r.get("faction") or faction,
                }

    # Holder — NPC near the context district who might hold the target
    holder_rows = _db_rows(
        "SELECT name, `rank`, motivation, quote, faction, role FROM npcs "
        "WHERE location LIKE %s AND status NOT IN ('dead','retired') ORDER BY RAND() LIMIT 1",
        (f"%{district}%",),
    )
    if not holder_rows:
        holder_rows = _db_rows(
    ...
    if holder_rows:
        ...
        seeds["holder"] = {
            "name": r["name"],
            "rank": r.get("rank") or r.get("role") or "local",
            "motivation": (r.get("motivation") or "")[:180],
            "quote": r.get("quote") or "",
            "faction": r.get("faction") or "",
        }

    # Rival party lead — from rival_party members or a random NPC from an opposing party
    rival_name = ""
    rival_members = rival_party.get("members") or []
    if isinstance(rival_members, list) and rival_members:
        rival_name = str(rival_members[0]) if isinstance(rival_members[0], str) else (rival_members[0].get("name") or "")
    if rival_name:
        rows = _db_rows(
            "SELECT name, `rank`, motivation, quote, faction FROM npcs "
            "WHERE LOWER(name) = LOWER(%s) LIMIT 1",
            (rival_name,),
        )
        if rows:
            ...
            seeds["rival"] = {
                "name": r["name"],
                "rank": r.get("rank") or "adventurer",
                "motivation": (r.get("motivation") or "")[:180],
                "quote": r.get("quote") or "",
                "faction": r.get("faction") or rival_party.get("name", ""),
            }
    if "rival" not in seeds:
        seeds["rival"] = {
            "name": rival_party.get("name") or "rival crew",
            "rank": "rival party lead",
            "motivation": f"Get to the target first and claim the contract pay.",
            "quote": "",
            "faction": "",
        }

    # 2-3 local NPCs near the district for witness/local lines
    local_rows = _db_rows(
        "SELECT name, `rank`, motivation, quote, role FROM npcs "
        "WHERE location LIKE %s AND status NOT IN ('dead','retired') ORDER BY RAND() LIMIT 3",
        (f"%{district}%",),
    )
    if not local_rows:
        local_rows = _db_rows(
            "SELECT name, `rank`, motivation, quote, role FROM npcs "
            "WHERE status NOT IN ('dead','retired') ORDER BY RAND() LIMIT 3"
        )
    seeds["locals"] = [
        {
            "name": r["name"],
            "rank": r.get("rank") or r.get("role") or "local",
            "motivation": (r.get("motivation") or "")[:120],
            "quote": r.get("quote") or "",
        }
        for r in local_rows
    ]

    # Current weather for local colour
    weather_rows = _db_rows("SELECT current_weather FROM weather_state LIMIT 1")
    seeds["weather"] = (weather_rows[0].get("current_weather") or "") if weather_rows else ""
...
async def _ollama(prompt: str, tokens: int = 3500) -> str:
    try:
...
async def _generate_plan(
    ...
    reward: Dict[str, Any],
    npc_seeds: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    ...
    # Build NPC voice block for the prompt
    _voice_lines: List[str] = []
    if npc_seeds:
        sp = npc_seeds.get("sponsor")
        if sp:
            _voice_lines.append(
                f"Sponsor NPC: {sp['name']} ({sp['rank'] or sp['faction']}) — "
                f"motivation: {sp['motivation'] or 'contract completion'} — "
                f"voice sample: {sp['quote'] or 'terse and transactional'}"
            )
        hl = npc_seeds.get("holder")
        if hl:
            _voice_lines.append(
                f"Holder NPC: {hl['name']} ({hl['rank']}) — "
                f"motivation: {hl['motivation'] or 'self-interest'} — "
                f"voice sample: {hl['quote'] or 'defensive and cagey'}"
            )
        rv = npc_seeds.get("rival")
        if rv:
            _voice_lines.append(
                f"Rival: {rv['name']} — motivation: {rv['motivation']} — "
                f"voice sample: {rv['quote'] or 'competitive and direct'}"
            )
        for loc in (npc_seeds.get("locals") or [])[:2]:
            _voice_lines.append(
                f"Local NPC: {loc['name']} ({loc['rank']}) — "
                f"voice sample: {loc['quote'] or 'wary and brief'}"
            )
        if npc_seeds.get("weather"):
            _voice_lines.append(f"City weather today: {npc_seeds['weather']} — locals reference it.")
    _voice_block = "\n".join(_voice_lines) if _voice_lines else "No live NPC data — invent plausible voices."
...
Live NPC voices (use these names and voice patterns in dialogue_bank):
{_voice_block}
...
- dialogue_bank: at least 16 lines using the REAL NPC names from "Live NPC voices" above. Write each line as "Name (role): [their exact words]". Make each line sound like the character's voice sample. Include 2-3 lines for sponsor, 2-3 for holder, 2 for rival, 2-3 for witnesses/locals, and 1-2 for clerk or bystanders. Reference the weather or city context where it fits.
...
- follow_up_hooks: 5 hooks"""
    ...
    for attempt in range(3):
        ...
        if data:
            break
        logger.warning(f"[RECOVERY] LLM returned no usable plan (attempt {attempt + 1}/3) for '{mission.get('title', 'Recovery')}'")
        await asyncio.sleep(2)
    if not data:
        logger.warning("[RECOVERY] LLM unavailable after retries; using mission-faithful fallback")
    return _normalize_plan(data, mission, target_type, context, dcs, rival_party, npc_seeds=npc_seeds)


def _pick_holder_npc(context: Dict[str, Any]) -> str:
    """Return a named NPC from DB near the context district, or a descriptive fallback."""
    district = context.get("district", "")
    try:
        rows = _db_rows(
            "SELECT name, role, faction FROM npcs WHERE status NOT IN ('dead','retired') "
            "AND location LIKE %s ORDER BY RAND() LIMIT 1",
            (f"%{district}%",),
        )
        if rows:
            r = rows[0]
            return f"{r['name']} ({r.get('role','') or r.get('faction','unknown')})"
        # Fallback: any living NPC
        rows2 = _db_rows("SELECT name, role FROM npcs WHERE status NOT IN ('dead','retired') ORDER BY RAND() LIMIT 1")
        if rows2:
            return f"{rows2[0]['name']} ({rows2[0].get('role','independent')})"
    except Exception:
        pass
    return "an unnamed holder who can be negotiated with, bypassed, or exposed"


def _build_fallback_dialogue(
    target_type: str,
    context: Dict[str, Any],
    rival_party: Dict[str, Any],
    npc_seeds: Optional[Dict[str, Any]] = None,
) -> List[str]:
    """Build a DB-seeded fallback dialogue bank using real NPC names and voice samples."""
    seeds = npc_seeds or {}
    sp   = seeds.get("sponsor") or {}
    hl   = seeds.get("holder") or {}
    rv   = seeds.get("rival") or {}
    locs = seeds.get("locals") or []
    weather = seeds.get("weather") or ""

    sp_name  = sp.get("name") or "Sponsor"
    sp_quote = sp.get("quote") or ""
    sp_rank  = sp.get("rank") or sp.get("faction") or "contact"

    hl_name  = hl.get("name") or "the holder"
    hl_quote = hl.get("quote") or ""
    hl_rank  = hl.get("rank") or "local"

    rv_name  = rv.get("name") or rival_party.get("name") or "rival crew"

    loc1 = locs[0] if locs else {}
    loc2 = locs[1] if len(locs) > 1 else {}
    loc3 = locs[2] if len(locs) > 2 else {}

    district = context.get("district") or context.get("name") or "the district"
    place    = context.get("name") or "the location"

    weather_ref = f" {weather}." if weather else ""

    # Sponsor lines — terse, contract-focused, in their voice
    sp_lines = [
        f"{sp_name} ({sp_rank}): " + (sp_quote if sp_quote else f"Payment is on return. Philosophy is unpaid."),
        f"{sp_name}: Damaged is not intact. Do not make me define the difference.",
        f"{sp_name}: I am not paying for the story of how it got away.",
    ]

    # Holder lines — defensive, self-interested
    hl_lines = [
        f"{hl_name} ({hl_rank}): " + (hl_quote if hl_quote else f"I did not take it. I found it. That is a different category."),
        f"{hl_name}: Possession is not theft if nobody came looking before you.",
        f"{hl_name}: Name your price. I have better things to protect than sentiment.",
    ]

    # Rival lines
    rv_lines = [
        f"{rv_name}: We can both walk away paid if you stop being heroic about it.",
        f"{rv_name}: Your sponsor posted first. Mine posted higher. Jurisdiction is unclear.",
    ]

    # Local lines — grounded, city-flavoured
    loc_lines: List[str] = []
    if loc1:
        l1q = loc1.get("quote") or f"Things go missing in {district} and then they stop being missing for the wrong person."
        loc_lines.append(f"{loc1.get('name', 'Local')} ({loc1.get('rank','local')}): {l1q}")
    else:
        loc_lines.append(f"Local ({district}): Things go missing here and then they stop being missing for the wrong person.")
    if loc2:
        l2q = loc2.get("quote") or f"I saw the mark.{weather_ref} I did not say I saw who carried it."
        loc_lines.append(f"{loc2.get('name', 'Witness')} ({loc2.get('rank','witness')}): {l2q}")
    else:
        loc_lines.append(f"Witness ({district}): I saw the mark. I did not say I saw who carried it.")
    if loc3:
        l3q = loc3.get("quote") or f"That thing made {place} feel colder when you named it out loud."
        loc_lines.append(f"{loc3.get('name', 'Bystander')}: {l3q}")
    else:
        loc_lines.append(f"Bystander ({district}): That thing made {place} feel colder when you named it out loud.")

    # Clerk and moral lines (generic but grounded)
    clerk_lines = [
        f"Clerk ({place}): The signature exists. The signer does not.",
        f"Clerk: I log what is handed to me. I do not log where it came from.",
    ]
    pet_lines = [] if target_type != "missing_pet" else [
        "Owner: Please. Just bring them home. The rest does not matter.",
        f"Owner: They always came back to {district}. That means something.",
    ]

    all_lines = sp_lines + hl_lines + rv_lines + loc_lines + clerk_lines + pet_lines
    return all_lines
...
def _fallback_plan(
    ...
    return {
        ...
        "retrieval_scene": {
            ...
            "current_holder": _pick_holder_npc(context),
        ...
        "dialogue_bank": _build_fallback_dialogue(target_type, context, rival_party, npc_seeds),
        "outcome_table": {
            "intact": "Full pay.",
            "usable": "Reduced pay.",
...
def _normalize_plan(
    ...
    fallback = _fallback_plan(mission, target_type, context, dcs, rival_party, npc_seeds=npc_seeds)
...
def render_module(
    ...
    return _page(title, body, faction, mission_id=mission.get("id"))
...
async def build_recovery_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    npc_seeds = _npc_seeds(mission, context, rival_party)
    ...
    plan = await _generate_plan(mission, target_type, context, dcs, strength, rival_party, reward, npc_seeds=npc_seeds)
    ...
    _html = render_module(mission, target_type, context, plan, dcs, strength, reward)
    ...
    # Pull a site map from the library (vault, archive, street — depends on target type)
    _rec_maps: list = []
    try:
        ...
        _map_type = {
            "evidence": "interior", "relic": "interior", "memory_identity": "interior",
            "data_record": "interior", "misplaced_cargo": "interior",
            "lost_gear": "dungeon", "missing_pet": "street",
        }.get(target_type, "interior")
        _rec_lib = copy_library_map_for_mission(
            mission, out_dir / "maps" / "recovery_site.png",
            mission_type="recovery", map_type=_map_type,
            district=context.get("district", ""),
        )
        if _rec_lib:
            _rec_maps = [_rec_lib]
            (out_dir / "maps.html").write_text(
                render_maps_page(title, faction, _rec_maps, "{}"), encoding="utf-8"
            )
    except Exception as _rec_me:
        logger.warning(f"[RECOVERY] Library map failed: {_rec_me}")

    from src.mission_builder.html_renderer import render_index
    cr = mission_cr(mission)
    index_html = render_index(title, faction, tier, cr, mission.get("player_name", "") or "Open", [], component_links(bool(_rec_maps)), None, len(_rec_maps))
    index_path = out_dir / "index.html"
    index_path.write_text(index_html, encoding="utf-8")

    try:
        from src.mission_builder.mission_db import write_module_slug_for_mission
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="RECOVERY")
    except Exception as e:
