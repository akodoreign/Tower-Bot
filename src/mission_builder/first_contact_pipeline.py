...
def _mission_context(mission: dict) -> Dict[str, Any]:
    ...
    for match in re.finditer(r"\b[A-Z][A-Za-z0-9'’.-]*(?:\s+[A-Z][A-Za-z0-9'’.-]*){0,4}\b", text):
        # Cut at the first sentence boundary so terms never swallow "Garden. Iron Fang".
        term = re.split(r"\.\s", match.group(0).strip())[0].strip()
        if term and term.lower() not in {"mission", "first contact", "standard", "posted"} and term not in terms:
            terms.append(term)
...
def _is_generic_plan(plan: Dict[str, Any], context: Dict[str, Any]) -> bool:
    # tower_primer / panic_meter default to module constants (TOWER_PRIMER says
    # "new arrivals") and must not trip the generic markers on every plan.
    check = {k: v for k, v in plan.items() if k not in ("tower_primer", "panic_meter")}
    ...
    if has_context and score >= 3:
        return False  # anchored in mission canon -- markers alone cannot reject it
...
def _pick_location() -> Dict[str, Any]:
    rows = _db_rows("SELECT district, place_type, name, type_tag, description, extra_json FROM gazetteer_places ORDER BY RAND() LIMIT 12")
...
async def _ollama(prompt: str, tokens: int = 2600) -> str:
    try:
        ...
        async with httpx.AsyncClient(timeout=240.0) as c:
            r = await c.post(OLLAMA_URL, json={"model": OLLAMA_MODEL, "messages": [{"role": "user", "content": prompt}], "stream": False, "options": {"temperature": 0.88, "num_predict": tokens, "num_ctx": _fit_ctx(prompt, tokens)}})
            r.raise_for_status()
...
Return JSON object only."""
    ...
    for attempt in range(3):
        ...
        if data:
            break
        logger.warning(f"[FIRST_CONTACT] LLM returned no usable plan (attempt {attempt + 1}/3) for '{mission.get('title', 'First Contact')}'")
        await asyncio.sleep(2)
    if not data:
        logger.warning("[FIRST_CONTACT] LLM unavailable after retries; using mission-faithful fallback")
...
def _fallback_plan(mission: dict, ctype: str) -> Dict[str, Any]:
    ...
    _first_term = (mission_context.get("canon_terms") or [title])[0]
    ...
    return {
        ...
        "first_sight": [
            "They stand under the Dome like it is a second moon.",
            "Their children count exits before learning names.",
            "Their clothes smell like rain from a sky that is not here.",
            "Their leader keeps touching the ground to make sure it remains ground.",
            "One of them clutches a tool that has no purpose here, and will not put it down.",
            "They speak in turns, oldest first, and go silent the moment anyone writes something down.",
            "When the artificial sky shifts, all of them stop moving at once.",
            f"Someone among them has already learned one Undercity word and repeats it quietly: the name {_first_term}.",
        ],
        ...
        "dialogue": [
            "New arrival: Is this a prison or a sky?",
            "Child: Why is your sun broken?",
            "Party line: EC is money. Kharma is harder to explain.",
            "Elder: Your factions sound like hungry houses.",
            "TNN: Can we get a statement?",
            "Warden: No cameras near the children.",
            f"Elder: We had a name for our home. {_first_term} is not it.",
            "New arrival: Your ground hums. Does it ever stop?",
            "Child: Are you the ones who took us, or the ones who found us?",
            "Party line: Nobody owns you here. Some people will try to convince you otherwise today.",
            "Elder: Who do we thank for water, and what do they take in exchange?",
            "New arrival: The tall spike in the middle. Does it watch us?",
            "Faction agent: We can offer protection, housing, and terms. Mostly terms.",
            "Party line: Do not sign anything you cannot read yet.",
            "Elder: Our dead came with us. Where do your dead go?",
            "Warden: Give them one quiet night before the city decides what they are.",
        ],
...
def render_session(mission: dict, plan: Dict[str, Any], dcs: Dict[str, int]) -> str:
    ...
    protocol_steps = plan.get("contact_protocol") or []
    protocol_html = "".join(
        f'<p><input type="checkbox"> <strong>{i}.</strong> {_e(s)}</p>'
        for i, s in enumerate(protocol_steps, 1)
    ) or "<p>No protocol defined — improvise safely.</p>"

    mis_steps = plan.get("misunderstanding_ladder") or []
    mis_html = (
        "<p><strong>Current state:</strong> <select style='font-family:inherit;'>"
        + "".join(f"<option>{_e(s)}</option>" for s in mis_steps)
        + "</select></p>"
        + "".join(f'<p><input type="checkbox"> Escalation: {_e(s)}</p>' for s in mis_steps)
    )

    translation_html = "".join(
        f'<p><input type="checkbox"> {_e(s)}</p>' for s in plan.get("translation_tracker", [])
    ) or "<p>No translation stages generated.</p>"

    panic_html = (
        "<p><strong>Panic level:</strong> <select style='font-family:inherit;'>"
        + "".join(f"<option>{_e(s)}</option>" for s in plan.get("panic_meter", []))
        + "</select></p>"
        + "<p><input type='checkbox'> Trust established &nbsp;"
        + "<input type='checkbox'> Consent to enter Tower system &nbsp;"
        + "<input type='checkbox'> Faction claim blocked</p>"
    )

    dc_rows = "".join(
        f'<tr><td style="padding:4px 8px;font-weight:600;">{_e(k.replace("_"," ").title())}</td>'
        f'<td style="padding:4px 8px;">DC {_e(v)}</td></tr>'
        for k, v in dcs.items()
    )
    dc_html = f'<table style="font-size:12px;border-collapse:collapse;">{dc_rows}</table>'

    risks = plan.get("faction_risks") or []
    risk_items = risks if isinstance(risks, list) else [str(risks)]
    risk_html = "".join(f'<p><input type="checkbox"> {_e(r)}</p>' for r in risk_items) or "<p>Monitor faction interest.</p>"

    dialogue = plan.get("dialogue") or []
    dialogue_html = "".join(
        f'<p style="font-size:12px;color:#444;margin:2px 0;"><em>{_e(line)}</em></p>'
        for line in dialogue[:8]
    ) or "<p><em>No dialogue generated.</em></p>"

    body = f'<h1 style="color:{fc};">{_e(title)}</h1>'
    body += _card("Contact Protocol", protocol_html, fc)
    body += _card("Misunderstanding Ladder", mis_html, "#7b1e1e")
    body += _card("Translation Progress", translation_html, "#2a6a2a")
    body += _card("Panic / Trust / Consent", panic_html, "#8a5a1f")
    body += _card("DC Reference", dc_html, "#3a6898")
    body += _card("Faction Risks", risk_html, "#7b1e1e")
    body += _card("Opening Dialogue (first 8)", dialogue_html, "#555")
    body += _card("Session Notes", "<textarea rows='5' style='width:100%;font-family:inherit;' placeholder='EC, Kharma, customs, faction moves, teaching moments...'></textarea>", "#8a5a1f")
    return _page(title, body, faction)
...
async def build_first_contact_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    _html = render_module(mission, ctype, location, plan, dcs, strength)
    ...
    # Pull a social/meeting-place map from the library
    _map_paths: list = []
    try:
        ...
        _district = plan.get("location", {}).get("district", "") if isinstance(plan.get("location"), dict) else ""
        _lib = copy_library_map_for_mission(mission, out_dir / "maps" / "contact_site.png",
                                            mission_type="first_contact", map_type="interior", district=_district)
        if _lib:
            _map_paths = [_lib]
            (out_dir / "maps.html").write_text(
                render_maps_page(title, faction, _map_paths, "{}"), encoding="utf-8"
            )
    except Exception as _me:
        logger.warning(f"[FIRST_CONTACT] Library map failed: {_me}")

    from src.mission_builder.html_renderer import render_index
    _clinks = component_links(bool(_map_paths))
    if _map_paths:
        _clinks.append(("maps", "maps.html"))
    index_html = render_index(title, faction, tier, mission_cr(mission), mission.get("player_name", "") or "Open", [], _clinks, None, len(_map_paths))
    index_path = out_dir / "index.html"
    index_path.write_text(index_html, encoding="utf-8")
    try:
        from src.mission_builder.mission_db import write_module_slug_for_mission
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="FIRST_CONTACT")
    except Exception as e:
