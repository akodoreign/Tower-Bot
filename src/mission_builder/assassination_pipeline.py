...
def _failed_recon_heat(host_faction: str = "") -> Optional[Dict[str, Any]]:
    """Read non-consuming failed recon that should harden an assassination site."""
    if not host_faction:
        return None
    try:
        ...
        rows = raw_query(
            ...
            "AND host_faction = %s ORDER BY created_at DESC LIMIT 1",
            (host_faction,),
        ) or []
    except Exception as exc:
        logger.debug(f"[ASSASSIN] failed recon lookup skipped: {exc}")
        return None
    if not rows:
        return None
    row = rows[0]
    raw = row.get("dossier_json")
    if isinstance(raw, str):
        try:
            row["dossier"] = json.loads(raw)
        except Exception:
            row["dossier"] = {}
    else:
        row["dossier"] = raw or {}
...
def _heighten_security(security: Dict[str, Any], failed_recon: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    if not failed_recon:
        return dict(security)
    hardened = dict(security)
    hardened["label"] = f"{security.get('label', 'Security')} (Heightened)"
    hardened["guards"] = f"{security.get('guards', '')}; extra watchers briefed on failed cover identities"
    hardened["patrol"] = f"{security.get('patrol', '')}; patrol pattern changes after each suspicious approach"
    hardened["alarm"] = f"{security.get('alarm', '')}; first false credential triggers silent tailing"
    hardened["escape_heat"] = f"{security.get('escape_heat', '')}; starts one step hotter due to prior failed recon"
    return hardened
...
def _failed_recon_heat_html(failed_recon: Optional[Dict[str, Any]]) -> str:
    ...
    venue = failed_recon.get("venue") or dossier.get("venue") or "the mark's security circle"
    event = failed_recon.get("event_name") or dossier.get("event") or "an earlier infiltration"
    details = failed_recon.get("details") or "The previous scout failed loudly enough to change procedures."
    return (
        ...
        f'{_e(details)} Guards now challenge repeated surveillance, social covers, and forged access earlier than normal.'
        f'</div>'
    )
...
async def _ollama(prompt: str, system: str = "") -> str:
    ...
    payload = {
        ...
        "stream":   False,
        "think":    False,
        "options":  {"temperature": 0.8, "num_predict": 1200, "num_ctx": _fit_ctx(system + prompt, 1200)},
    }
...
async def _generate_assassination_map(
    ...
    """Select one private-interior tactical map from the battle map library."""
    ...
    return copy_library_map_for_mission(
        {"faction": target.get("faction", "") or target.get("target_faction", "")},
        out,
        mission_type="assassination",
        location_name=target.get("location", "") or target.get("target_location", "") or hit_site,
        description=f"{hit_site} {target.get('target_name', '')}",
        map_type="office",
    )
...
def _assassination_methods(base: int) -> List[Dict[str, Any]]:
    """The ways the hit can go down, each a skill check (with options) plus setup
    and success/failure. D&D 5.5e (2024) accurate. DCs scale to party level."""
    return [
        {"method": "Poison", "dc": base + 1,
         "skills": "Dexterity (Sleight of Hand) to dose; Poisoner's Kit or Wisdom (Medicine) to prepare",
         "setup": "Slip a toxin into the mark's food, drink, cup, or gear during the weak window.",
         "success": "The mark fails a Constitution save away from help; it reads as sudden illness, not murder.",
         "failure": "Wrong dose or a watchful taster -- the mark is spooked, or the wrong person goes down."},
        {"method": "Blade in the Dark", "dc": base + 1,
         "skills": "Dexterity (Stealth) to close unseen; the strike itself while hidden",
         "setup": "Cross the gap during the blind moment and strike before the mark can react.",
         "success": "One decisive strike from hiding (Advantage; a rogue's Sneak Attack applies) ends it quietly.",
         "failure": "A scuffed approach -- the mark cries out and the guards converge."},
        {"method": "Staged Accident", "dc": base + 2,
         "skills": "Intelligence (Investigation) to find the weak point; Thieves' Tools or Sleight of Hand to rig it",
         "setup": "Engineer a death that looks like misfortune -- a loose rail, a faulty lamp, a cut strap.",
         "success": "No blade, no blame; it reads as an accident and escape heat stays low.",
         "failure": "The rig is found or fires early -- now it is plainly murder, and the heat climbs."},
        {"method": "Ranged Shot", "dc": base + 2,
         "skills": "Dexterity (a ranged weapon); Wisdom (Perception) or Survival to find the perch and the line",
         "setup": "Take an overwatch position along the routine and wait for the clean line.",
         "success": "One shot from concealment; you are gone before anyone traces the angle.",
         "failure": "The shot is blocked or wide -- the mark knows they are hunted and goes to ground."},
        {"method": "Arcane", "dc": base + 1,
         "skills": "Intelligence (Arcana) or your spellcasting; Subtle Spell to cast unseen",
         "setup": "End it with magic -- a quiet spell, a cursed object, a hexed drink.",
         "success": "Clean and deniable if no one saw the casting (Subtle Spell removes the verbal/somatic tells).",
         "failure": "A witnessed casting points straight at an arcane killer -- and to magical forensics."},
    ]


def _method_table(methods: List[Dict[str, Any]]) -> str:
    head = (
        '<table style="width:100%;border-collapse:collapse;font-size:13px;">'
        '<tr>'
        '<th style="text-align:left;padding:6px 8px;background:#2a2218;color:#e8dcc8;">Method</th>'
        '<th style="text-align:left;padding:6px 8px;background:#2a2218;color:#e8dcc8;">Check</th>'
    ...
    rows = "".join(
        ...
        f'<td style="padding:6px 8px;font-weight:bold;vertical-align:top;">{_e(m["method"])}</td>'
        f'<td style="padding:6px 8px;vertical-align:top;">{_e(m["skills"])} <span style="white-space:nowrap;">(DC {m["dc"]})</span></td>'
        f'<td style="padding:6px 8px;vertical-align:top;">{_e(m["setup"])}</td>'
        f'<td style="padding:6px 8px;vertical-align:top;color:#2a6a2a;">{_e(m["success"])}</td>'
        f'<td style="padding:6px 8px;vertical-align:top;color:#7b1e1e;">{_e(m["failure"])}</td>'
        f'</tr>'
        for m in methods
    )
...
def render_assassination_module(
    ...
    out_dir: Path,
    prior_recon: Optional[dict] = None,
    failed_recon: Optional[dict] = None,
) -> str:
    ...
    if prior_recon:
        ...
        _vn = prior_recon.get("venue") or "the mark's circle"
        _oc = prior_recon.get("outcome", "scouted")
        _note = (" The intel is solid -- use it to skip a surveillance lead."
                 if _oc == "success" else
                 " The scout was blown; treat the intel as good-but-incomplete."
                 if _oc == "partial" else
                 " The scout went sideways; verify before you commit.")
        recon_banner = (
            f'<div style="background:#eef6ff;border:1px solid #3a6898;border-radius:6px;padding:8px 12px;margin:12px 0;font-size:13px;">'
    ...
    # Method spine -- DC scaled to party level (defensive against strength shape).
    try:
        _ps = _party_strength()
        _avg = int(_ps.get("avg_level", 5)) if isinstance(_ps, dict) else 5
    except Exception:
        _avg = 5
    methods_html = _method_table(_assassination_methods(12 + max(0, (_avg - 3) // 2)))
    ...
    body = f"""
{recon_banner}
{_failed_recon_heat_html(failed_recon)}
...
<h2>Method Options - How the Hit Goes Down</h2>
<div style="font-size:12px;color:#666;margin-bottom:8px;">Each method is a skill check (with options) and its own setup and success/failure. Pick what fits the recon and the party -- a clean method keeps escape heat low; a loud one does not.</div>
{methods_html}
...
async def build_assassination_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    failed_recon = _failed_recon_heat(mission.get("opposing_faction", "") or faction)
    security     = _heighten_security(security, failed_recon)

    logger.info(f"[ASSASSIN] Security={security['label']} | site={hit_site}")

    body = mission.get("body") or mission.get("description") or ""
    target_name_hint = mission.get("target_npc_name") or _extract_target_name(body)
    if target_name_hint:
    ...
    from src.mission_builder.mimir_module import (
        ...
        enrich_mission_loot as _mel,
    )
    _mimir_id = await _mc(mission, mission_type="assassination")
    if _mimir_id and map_path:
        await _mu(_mimir_id, map_path, "Hit Site Map")

    # Build on a prior infiltration's recon (explicit parent_recon_id if spawned,
    # else best match for the mark's faction).
    prior_recon = None
    try:
        ...
        if not prior_recon:
            prior_recon = find_recon_for_followup(host_faction=mission.get("opposing_faction", "") or faction)
    except Exception as _recon_err:
        logger.debug(f"[ASSASSIN] recon lookup skipped: {_recon_err}")

    # Module HTML
    module_html = render_assassination_module(
        mission, briefing, target, security, surveillance, approaches, exfil, map_path, out_dir, prior_recon, failed_recon,
    )
    ...
    if prior_recon and prior_recon.get("id"):
        try:
            ...
            logger.debug(f"[ASSASSIN] recon consume skipped: {_recon_err}")
    ...
    dm_md = (
        ...
        + (
            ...
            f"- Effect: guards challenge repeated surveillance, forged access, and cover identities earlier than normal.\n\n"
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="ASSASSIN")
