...
async def _ollama(prompt: str, tokens: int = 2500) -> str:
    payload = {
        "model": OLLAMA_MODEL,
...
def _research_table(research: List[Dict[str, Any]]) -> str:
    ...
    return (
        "<table><tr><th>Lane</th><th>Where</th><th>Check</th><th>Find</th><th>Marker</th><th>Risk</th></tr>"
        + rows + "</table>"
    )
...
def render_negotiation_session(mission: dict, roles: Dict[str, str], plan: Dict[str, Any], marker: int, dcs: Dict[str, int]) -> str:
    ...
    body += _card("Research / Favor Tracker", "<textarea rows='6' style='width:100%;font-family:inherit;' placeholder='Research used, favors called, marker shifts...'></textarea>", "#2a6a2a")
    body += _card("Skill DCs", _table([("Easy", dcs["easy"]), ("Standard", dcs["standard"]), ("Hard", dcs["hard"]), ("Severe", dcs["severe"])]), "#8a5a1f")
    body += _card("Result", "<p><strong>Final marker:</strong> <input type='number' min='-10' max='10' style='width:60px;'></p><p><strong>Agreement signed:</strong> <input type='checkbox'></p><p><strong>Follow-up spawned:</strong> <input type='text'></p><textarea rows='4' style='width:100%;font-family:inherit;' placeholder='Debrief notes...'></textarea>", "#7b1e1e")
    return _page(title, body, roles["sponsor"])
...
def _chart_pack_md(plan: Dict[str, Any], dcs: Dict[str, int], marker: int) -> str:
    return (
        ...
        + "\n".join(f"| {r.get('lane')} | {r.get('result')} | {r.get('marker_effect')} | {r.get('risk')} |" for r in plan.get("research", []))
