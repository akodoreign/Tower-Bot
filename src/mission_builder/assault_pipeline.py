  ...
  "commander_dialogue": ["line spoken when battle starts", "line spoken when morale breaks", "line spoken when cornered or surrendering"]
    ...
    if not data:
        data = {
            ...
            "position_approach_read_aloud": (lambda: __import__('src.mission_builder.read_aloud_library', fromlist=['scene_read_aloud']).scene_read_aloud("s1", mission_type="assault", contact="the commander", location=position))(),
            "commander_dialogue": [
...
async def _generate_assault_map(
    ...
    return copy_library_map_for_mission(
        ...
        description=" ".join(cp.get("name", "") for cp in chokepoints if cp.get("name")),
    )
...
async def build_assault_module(mission: dict, out_dir: Optional[Path] = None) -> Path:
    ...
    try:
        ...
        write_module_slug_for_mission(mission, out_dir.name, log_prefix="ASSAULT")
    except Exception as _e:
