...
_REDDIT_COOKIE  = os.getenv("REDDIT_COOKIE", "")
_REDDIT_TOKEN   = os.getenv("REDDIT_TOKEN", "")   # token_v2 value (expires ~24h)
...
if _REDDIT_COOKIE:
    REDDIT_HEADERS["Cookie"] = _REDDIT_COOKIE
if _REDDIT_TOKEN:
    REDDIT_HEADERS["Authorization"] = f"Bearer {_REDDIT_TOKEN}"
...
async def fetch_posts(
    ...
    try:
        async with httpx.AsyncClient(
            headers=REDDIT_HEADERS, timeout=30.0, follow_redirects=True
        ) as c:
...
async def download_image(url: str, out_path: Path) -> bool:
    ...
    try:
        async with httpx.AsyncClient(
            headers={**REDDIT_HEADERS, "Accept": "image/*"},
            timeout=60.0,
...
async def run(
    ...
    for tf in timeframes:
        ...
        while True:
            ...
            posts, after = await fetch_posts(after=after, timeframe=tf)
            if not posts:
                break
            pages += 1
