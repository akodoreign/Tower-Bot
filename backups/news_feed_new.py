...
ROSTER NPCs (use for conflict/drama pieces — do NOT kill them in bulletins):
- Sera Voss (Half-Elf, Iron Fang Consortium, Senior Agent, Crimson Alley) — ambitious, skimming funds, building private network
- Corvin Thale (Human, Wardens of Ash, Lt-Inspector, Outer Wall) — internal affairs, building a file on Korin
- Mira Kessan (Human, Independent info broker, Grand Forum) — secretly feeds FTA sanitised intel
- Brother Aldric (Human, Thane's Cult, enforcer, Collapsed Plaza) — walked a Rift, faith cracked, hiding what he saw
- Yuna Saltis (Tiefling, Obsidian Lotus, Memory Architect, Night Pits) — collects erased memories secretly
- Pol Greaves (Dwarf, Patchwork Saints, Field Captain, Scrapworks) — old failed adventurer, hiding a grudge
- Tessaly Orin (Half-Elf, Guild of Ashen Scrolls, Archivist, Grand Forum Library) — discovered narrative resonance pattern
- Rook (Unknown species, freelance finder, Guild Spires adjacent) — unknown nature, no faction loyalty
- Calix Drenn (Human, FTA Field Officer, Grand Forum) — sister in Thane's cult, not reported it
- Vesper (Tiefling, Serpent Choir, Contract Mediator, Sanctum Quarter) — under an unknown divine contract
- Gorgon Gizzick (Goblin, Thane's Cult, Speaker, Collapsed Plaza) — secretly worships a god Thane banished\
"""
...
def _build_prompt(memory_entries: List[str]) -> str:
    ...
    _ROSTER_NPC_NAMES = [
        "Sera Voss", "Corvin Thale", "Mira Kessan", "Brother Aldric", "Yuna Saltis",
        "Pol Greaves", "Tessaly Orin", "Rook", "Calix Drenn", "Vesper", "Gorgon Gizzick",
    ]
    alive_roster = [n for n in _ROSTER_NPC_NAMES if n not in dead_names]
    if len(alive_roster) >= 2:
        roster_list = ", ".join(alive_roster)
        risky_roster = [n for n in ["Sera Voss", "Pol Greaves", "Brother Aldric", "Calix Drenn", "Gorgon Gizzick"] if n in alive_roster]
...
async def _build_image_prompt(memory_entries: List[str]) -> tuple:
    ...
    if is_anime:
        quality_header = (
            ...
            "fate/stay night \\(anime\\), type-moon, ufotable, official art, anime screencap"
        )
...
async def generate_story_image() -> tuple:
    ...
    if is_anime:
        _style_suffix = (
            ", masterpiece, best quality, very aesthetic, absurdres, "
            "fate/stay night \\(anime\\), type-moon, ufotable, "
            "official art, anime screencap, "
            "dramatic lighting, volumetric lighting, lens flare, particle effects, "
            "intricate background, detailed environment, dark fantasy, "
