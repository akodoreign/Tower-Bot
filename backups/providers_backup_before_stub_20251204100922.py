...
class FreeProvider:
    """
    Disabled stub for the old g4f-based free provider.

    This avoids crashes when g4f removes or renames providers like `Chatai`
    or `CohereForAI_C4AI_Command`. The rest of the bot can still instantiate
    FreeProvider, but any attempt to actually use it will raise a clear error.
    """
    ...
    def __init__(self):
        ...
        self.default_model_name = "disabled"

    async def acreate(self, *args, **kwargs):
        raise RuntimeError(
            "Free provider is disabled. Configure a premium provider (e.g. OpenAI) "
            "and use /provider to switch to it."
        )

    def get_model_list(self):
        # Return empty list so menus don’t break
        return []
