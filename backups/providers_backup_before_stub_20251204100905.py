...
class FreeProvider(BaseProvider):
    ...
    def __init__(self):
        ...
        for provider_info in self.working_providers:
            logger.info(f"  âœ… {provider_info['name']}: {', '.join(provider_info['models'])}")
        ...
        for attempt in range(len(self.working_providers)):
            ...
            try:
                ...
                if response and response.choices and response.choices[0].message.content:
                    ...
                    logger.info(f"âœ… Success with {provider_info['name']} + {provider_model}")
                    return result
                else:
                ...
                error_msg = str(e)
                logger.warning(f"âŒ {provider_info['name']} failed: {error_msg[:100]}...")
