...
class FreeProvider:
    """
    Working free provider using g4f.

    This avoids referencing specific g4f.Provider backends (like `Chatai`)
    that keep changing. Instead, it calls g4f's ChatCompletion interface
    directly, letting g4f pick an available free backend.
    """

    def __init__(self):
        # No per-backend config; g4f will route the request
        self.providers = []
        self.working_providers = []
        self.current_index = 0
        # Name used internally by the bot; it's mostly cosmetic for Free
        self.default_model_name = "gpt-free"

    async def acreate(self, messages, model_name=None, **kwargs):
        """
        Asynchronous entrypoint used by the bot.

        `messages` should be an OpenAI-style list:
        [
            {"role": "system", "content": "..."},
            {"role": "user", "content": "..."},
            ...
        ...
        # g4f ChatCompletion API - this is synchronous, but we can call it
        # directly from async; worst case it blocks a bit, which is fine
        response = g4f.ChatCompletion.create(
            model="gpt-4.0-mini",  # g4f will map this to a free backend
            messages=messages,
        )

        # The rest of the bot expects an OpenAI-style response object
        return response

    def get_model_list(self):
        # Return empty list so provider menus don't break; Free models
        # are basically "one size" here.
        return []
