...
class DiscordClient(discord.Client):
    def __init__(self) -> None:
        ...
        try:
            ...
            self.provider_manager.set_current_provider(ProviderType.OPENAI)
    ...
    async def handle_response(self, user_message: str) -> str:
        ...
        try:
            ...
            if self.provider_manager.current_provider != ProviderType.OPENAI:
                ...
                try:
                    free_provider = self.provider_manager.get_provider(ProviderType.OPENAI)
                    response = await free_provider.chat_completion(
                        messages=self.conversation_history,
                        model=None
                    )
                    self.conversation_history.append({'role': 'assistant', 'content': response})
                    return f"{response}\n\n*âš ï¸ Fallback to free provider due to error*"
                except Exception as fallback_error:
                    logger.error(f"Fallback provider also failed: {fallback_error}")
                    # Return user-friendly error message
                    error_response = "âŒ I'm having trouble processing your request right now. Please try again later or contact an administrator."
                    self.conversation_history.append({'role': 'assistant', 'content': error_response})
                    return error_response
            else:
                # Already using free provider, return error
                error_response = "âŒ The free provider is currently unavailable. Please try again later."
                self.conversation_history.append({'role': 'assistant', 'content': error_response})
                return error_response
