...
| `db_api.py` | MySQL pool; raw_query, raw_execute, get/set_global_state | ACTIVE |
...
| File | Purpose | Status |
|------|---------|--------|
| `npc_lifecycle.py` | NPC lifecycle events (death, injury, retirement) | ACTIVE |
| `npc_appearance.py` | NPC visual description gen | ACTIVE |
| `npc_consequence.py` | Post-bulletin NPC consequence scanner | ACTIVE |
| `npc_lookup.py` | NPC name→context for chat | ACTIVE |
| `npc_statblock_backfill.py` | D&D stat block backfill | ACTIVE |
| `character_profiles.py` | PC profile management | ACTIVE |
| `character_monitor.py` | Memory + consequence polling (uses character_memory/ dir) | ACTIVE |
| `party_profiles.py` | Multi-player party info | ACTIVE |
...
## Data Directories
- `campaign_docs/` — Legacy JSON (read-only; tower_rag.py reads some; DB is authoritative)
