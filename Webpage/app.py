...
def api_missions():
    ...
    try:
        ...
        # For completed/failed missions, join debrief outcomes by title
        if rows and tab in ("completed", "failed", "all"):
            ...
            if titles:
                ...
                outcome_rows = raw_query(
                    ...
                    f"FROM mission_outcomes WHERE mission_title IN ({placeholders}) "
                    f"ORDER BY created_at DESC",
                    tuple(titles),
                ) or []
                outcome_map = {o["mission_title"]: o for o in outcome_rows}
                for r in rows:
                    r["_outcome"] = outcome_map.get(r.get("title", ""), {})
    ...
    try:
        ...
        if title:
            outcome_rows = raw_query(
                ...
                "FROM mission_outcomes WHERE mission_title = %s ORDER BY created_at DESC LIMIT 1",
                (title,),
            ) or []
...
def api_claim_mission():
    ...
    try:
        ...
        queue = get_global_state("dashboard_claim_queue") or []
        if not isinstance(queue, list):
            queue = []
        entry = {"mission_id": int(mission_id), "claimer": "DM Dashboard"}
        if not any(item.get("mission_id") == int(mission_id) for item in queue):
            queue.append(entry)
            set_global_state("dashboard_claim_queue", queue)

        return jsonify({"ok": True, "queued": True, "position": len(queue), "handoff": "discord_bot"})
...
def _serialize_npc(r: dict) -> dict:
    ...
    motivation = _pick_text(data.get("motivation"), app_data.get("motivation"))
    relationships = _pick_text(data.get("relationships"), app_data.get("relationships"))
    secret = _pick_text(data.get("secret"), app_data.get("secret"))
    oracle_notes = _pick_text(data.get("oracle_notes"), app_data.get("oracle_notes"))
    ...
    return {
        ...
        "level": level,
        "rank": _pick_text(data.get("rank"), app_data.get("rank")),
        ...
        "quote": _npc_quote({**data, "name": name, "faction": faction, "role": role}, stats),
        "bio": public_notes,
        "stats": {
            "ac": stats.get("AC"),
            "hp": stats.get("HP"),
...
def api_factions():
    try:
        ...
        rep_map = {r["faction_name"]: r for r in rows}
        result = []
        for f in FACTIONS:
            rep = rep_map.get(f, {})
            result.append({
...
def api_generate_mission():
    ...
    from src.mission_board import _LORE, _build_npc_context, _load_area_context_block
    ...
    try:
        ...
        location_name, _ = find_location_for_mission(faction=faction or None, tier=tier)
...
MISSION PARAMETERS:
- Faction: {faction or "any appropriate faction"}
...
- BANNED words in titles: Harvest, Shadow, Shadows, Darkness, Silence, Reckoning, Unraveling, Corruption, Awakening, Legacy, Revelation, Convergence, Resonance, Void, Abyss, Storm, Tide, Forgotten, Ancient, Flame, Crimson, Obsidian (unless it IS the Obsidian Lotus faction name)
...
- If Mission type is Heist, the faction must be a shady sponsor such as Obsidian Lotus, Iron Fang Consortium, Glass Sigil, Argent Blades, Serpent Choir, or Brother Thane's Cult.
...
def api_complete_mission():
    ...
    if not title:
        return jsonify({"ok": False, "error": "mission_title required"}), 400

    try:
        from src.db_api import raw_query, raw_execute, db
        from datetime import datetime

        # Write to mission_outcomes
        existing = raw_query(
            "SELECT id FROM mission_outcomes WHERE mission_title = %s LIMIT 1", (title,)
        )
        outcome_fields = {
            "mission_title":    title,
            "result":           result,
        ...
        if existing:
            set_clause = ", ".join(f"{k}=%s" for k in outcome_fields if k != "mission_title")
            vals = [v for k, v in outcome_fields.items() if k != "mission_title"] + [title]
            raw_execute(
                f"UPDATE mission_outcomes SET {set_clause} WHERE mission_title=%s", vals
            )
        else:
            db.insert("mission_outcomes", outcome_fields)

        # Flip mission status in missions table
        ...
        if mid:
            ...
            raw_execute(
                "UPDATE missions SET status=%s WHERE title=%s",
                (status_val, title),
            )

        app.logger.info(f"📋 Session debrief submitted: '{title}' → {status_val}")
        return jsonify({"ok": True, "status": status_val})
...
def api_image_refs():
    try:
        ...
        for r in rows:
            ...
            category = "npcs_alt" if r.get("entity_type") == "npc_portrait_alt" else "npcs" if r.get("entity_type") == "npc_portrait" else "locations"
...
def serve_project_media(filename: str):
    """Serve generated project media, limited to files inside this checkout."""
    full = (PROJECT_ROOT / filename).resolve()
    if not _path_within(full, PROJECT_ROOT):
...
def _mimir_run(coro):
    """Run a Mimir async coroutine synchronously from Flask."""
    import asyncio
    loop = asyncio.new_event_loop()
    try:
